#!/usr/bin/env python3
"""Authenticated loopback event client for a future AITrain V2 Worker Host.

Wire contract:
1. the first JSONL frame is ``{"channel":"aitrain.adapter.v2","token":"..."}``;
2. the host replies ``{"status":"accepted"}`` before any task event is sent;
3. all following frames are Protocol V2 envelopes, one compact JSON object per
...
Worker accept V2 adapter connections.
"""
...
PROTOCOL_VERSION = 2
CHANNEL_NAME = "aitrain.adapter.v2"
...
class AdapterEventChannelError(RuntimeError):
    """The V2 adapter event channel could not be established or used."""
...
class AdapterEventChannelV2:
    ...
    def __enter__(self) -> "AdapterEventChannelV2":
        self.connect()
        return self
    ...
    def send_event(self, kind: str, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if kind not in _KNOWN_KINDS:
            raise ValueError(f"unsupported Protocol V2 event kind: {kind}")
        self._sequence += 1
        event = {
        ...
        if len(wire) > max_bytes:
            self._sequence -= 1
            raise ValueError(f"Protocol V2 message exceeds maximum size for kind {kind}")
        try:
    ...
    def emit_legacy_event(self, event: Mapping[str, Any]) -> dict[str, Any]:
        """Bridge the SDK's current event naming to a Protocol V2 envelope."""
...
def event_channel_from_environment() -> AdapterEventChannelV2:
    """Load a V2 event channel only from explicit Worker Host environment."""
    required = {
    ...
    if missing:
        raise AdapterEventChannelError(f"missing V2 event channel environment values: {', '.join(missing)}")
    try:
        port = int(str(required["port"]))
    except ValueError as exc:
        raise AdapterEventChannelError("AITRAIN_EVENT_PORT must be an integer") from exc
    return AdapterEventChannelV2(
