...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("ONNX 会写入 AITrain sidecar；NCNN 依赖本机 onnx2ncnn；TensorRT 需兼容 GPU/runtime，不兼容硬件会记录为 hardware-blocked。"), QStringLiteral("ONNX writes an AITrain sidecar. NCNN depends on local onnx2ncnn. TensorRT requires compatible GPU/runtime; incompatible hardware is recorded as hardware-blocked.")},
