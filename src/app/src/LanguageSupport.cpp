...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("PaddleOCR Rec（PP-OCRv4/v5 官方）"), QStringLiteral("PaddleOCR Rec (PP-OCRv4/v5 Official)")},
        ...
        {QStringLiteral("当前模型能力：官方 PaddleOCR PP-OCRv4/v5 Rec 适配器。通过模型预设选择版本，适合隔离 OCR Python 环境，记录 train/export/predict 命令、checkpoint、inference model 和官方预测报告。"), QStringLiteral("Current capability: official PaddleOCR PP-OCRv4/v5 Rec adapter. Select the version through the model preset; best used in an isolated OCR Python environment; records train/export/predict commands, checkpoints, inference models, and official prediction reports.")},
        {QStringLiteral("当前模型能力：官方 PaddleOCR PP-OCRv4/v5 Det 适配器。通过模型预设选择版本，适合 PaddleOCR 原生 det_gt.txt 数据，输出官方配置、checkpoint、inference model 和报告。"), QStringLiteral("Current capability: official PaddleOCR PP-OCRv4/v5 Det adapter. Select the version through the model preset; suitable for native PaddleOCR det_gt.txt data; outputs official config, checkpoint, inference model, and report.")},
