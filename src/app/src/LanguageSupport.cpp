...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("管理插件、界面语言、默认目录、授权状态和本地路径"), QStringLiteral("Manage plugins, interface language, default folders, license status, and local paths.")},
        {QStringLiteral("管理插件、界面语言、默认目录、授权状态和本地路径。"), QStringLiteral("Manage plugins, interface language, default folders, license status, and local paths.")},
        ...
        {QStringLiteral("打开插件设置"), QStringLiteral("Open Plugin Settings")},
        ...
        {QStringLiteral("诊断包包含 Worker self-check、环境 profile、GPU/驱动、最近任务日志、失败请求、artifact index、插件状态和授权摘要。"), QStringLiteral("The diagnostics bundle includes Worker self-check, environment profile, GPU/driver details, recent task logs, failed requests, artifact index, plugin status, and license summary.")},
        ...
        {QStringLiteral("插件市场：等待加载本地索引。"), QStringLiteral("Plugin marketplace: waiting to load local index.")},
        {QStringLiteral("市场"), QStringLiteral("Marketplace")},
        {QStringLiteral("插件扩展"), QStringLiteral("Plugin Extensions")},
        {QStringLiteral("已加载"), QStringLiteral("Loaded")},
        {QStringLiteral("插件市场"), QStringLiteral("Plugin Marketplace")},
        {QStringLiteral("检查 NVIDIA 驱动、CUDA、TensorRT、ONNX Runtime、Qt 插件和 Worker 可用性，并集中查看交付证据、诊断包和客户域 OCR 验收。"), QStringLiteral("Check NVIDIA driver, CUDA, TensorRT, ONNX Runtime, Qt plugins, and Worker availability, and review delivery evidence, diagnostics bundles, and customer-domain OCR acceptance in one place.")},
        ...
        {QStringLiteral("暂无市场插件"), QStringLiteral("No marketplace plugins")},
        {QStringLiteral("请加载 plugins/marketplace/marketplace.json 或内网静态索引。"), QStringLiteral("Load plugins/marketplace/marketplace.json or an internal static index.")},
        {QStringLiteral("选择插件包"), QStringLiteral("Choose Plugin Package")},
        {QStringLiteral("AITrain 插件包 (*.aitrain-plugin.zip *.zip);;所有文件 (*.*)"), QStringLiteral("AITrain Plugin Package (*.aitrain-plugin.zip *.zip);;All Files (*.*)")},
        {QStringLiteral("请先在“已安装”表格中选择插件。"), QStringLiteral("Select a plugin in the Installed table first.")},
        {QStringLiteral("插件已启用：%1 %2"), QStringLiteral("Plugin enabled: %1 %2")},
        {QStringLiteral("插件已禁用：%1"), QStringLiteral("Plugin disabled: %1")},
        {QStringLiteral("插件已卸载：%1 %2"), QStringLiteral("Plugin uninstalled: %1 %2")},
        {QStringLiteral("暂无已安装市场插件"), QStringLiteral("No installed marketplace plugins")},
        {QStringLiteral("可通过“导入插件包”安装本地插件包。"), QStringLiteral("Use Import Plugin Package to install a local plugin package.")},
        {QStringLiteral("插件市场：已安装 %1 个，状态文件 %2"), QStringLiteral("Plugin marketplace: %1 installed, state file %2")},
        {QStringLiteral("加载索引"), QStringLiteral("Load Index")},
        {QStringLiteral("市场索引"), QStringLiteral("Marketplace Index")},
        {QStringLiteral("分类"), QStringLiteral("Category")},
        {QStringLiteral("发布者"), QStringLiteral("Publisher")},
        {QStringLiteral("启用"), QStringLiteral("Enable")},
        {QStringLiteral("安装目录"), QStringLiteral("Install Folder")},
        {QStringLiteral("导入插件包"), QStringLiteral("Import Plugin Package")},
        {QStringLiteral("禁用"), QStringLiteral("Disable")},
        {QStringLiteral("卸载"), QStringLiteral("Uninstall")},
        {QStringLiteral("打开市场目录"), QStringLiteral("Open Marketplace Folder")},
