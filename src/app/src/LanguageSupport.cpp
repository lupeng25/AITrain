...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("YOLO 检测/分割指标来自 Ultralytics official val()。AITrain 不再生成本地 AP/mAP、mask IoU、混淆矩阵、错误样本或 overlay。"), QStringLiteral("YOLO detection/segmentation metrics come from Ultralytics official val(). AITrain no longer generates local AP/mAP, mask IoU, confusion matrices, error samples, or overlays.")},
        ...
        {QStringLiteral("C++ ONNX / NCNN 仅支持 YOLO detection / segmentation；TensorRT engine 使用部署验证；OCR 只查看 PaddleOCR 官方工具链产物。"), QStringLiteral("C++ ONNX / NCNN supports only YOLO detection / segmentation. TensorRT engines use deployment validation. OCR is viewed through PaddleOCR official toolchain artifacts.")},
        ...
        {QStringLiteral("推理验证解析 YOLO ONNX / NCNN param 产物；TensorRT engine 当前仅做部署验证状态记录；OCR 端到端结果通过 PaddleOCR 官方任务产物查看。"), QStringLiteral("Inference validation parses YOLO ONNX / NCNN param artifacts. TensorRT engines currently record deployment validation status only. OCR end-to-end results are viewed through PaddleOCR official task artifacts.")},
        ...
        {QStringLiteral("完成后显示检测框或分割 mask；OCR 可视化图来自 PaddleOCR 官方任务产物。"), QStringLiteral("After completion, shows detection boxes or segmentation masks; OCR visualization comes from PaddleOCR official task artifacts.")},
        {QStringLiteral("暂无 overlay\n运行推理后显示可视化产物。"), QStringLiteral("No overlay\nRun inference to show the visualization artifact.")},
        {QStringLiteral("推理验证仅选择 YOLO detection / segmentation 后处理；完整 OCR 推理通过 PaddleOCR 官方工具链任务产物查看。"), QStringLiteral("Inference validation selects only YOLO detection / segmentation postprocess. Full OCR inference is viewed through PaddleOCR official toolchain task artifacts.")},
        ...
        {QStringLiteral("检测框、分割 mask 和耗时会显示在这里；OCR 文本来自 PaddleOCR 官方报告。"), QStringLiteral("Detection boxes, segmentation masks, and latency appear here; OCR text comes from PaddleOCR official reports.")},
        ...
        {QStringLiteral("插件目录"), QStringLiteral("Plugin Folder")},
        {QStringLiteral("插件市场目录"), QStringLiteral("Plugin Marketplace Folder")},
