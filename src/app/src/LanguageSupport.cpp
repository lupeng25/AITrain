...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("C++ ONNX 仅支持 YOLO detection / segmentation；OCR 只查看 PaddleOCR 官方工具链产物。"), QStringLiteral("C++ ONNX supports only YOLO detection / segmentation; OCR is viewed through PaddleOCR official toolchain artifacts.")},
        ...
        {QStringLiteral("推理验证仅解析 YOLO ONNX / TensorRT 产物；OCR 端到端结果通过 PaddleOCR 官方任务产物查看。"), QStringLiteral("Inference validation parses only YOLO ONNX / TensorRT artifacts; OCR end-to-end results are viewed through PaddleOCR official task artifacts.")},
