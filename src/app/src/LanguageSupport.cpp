...
QString fallbackEnglishTranslation(const QString& source)
{
    static const QHash<QString, QString> translations = {
        ...
        {QStringLiteral("训练产物可从“任务与产物”注册为模型版本；评估、基准、流水线与交付报告均通过 Worker 执行。"), QStringLiteral("Training artifacts can be registered as model versions from Tasks and Artifacts; evaluation, benchmarking, pipelines, and delivery reports all run through the Worker.")},
