...
class TaskExecutionHostV2 final {
public:
    ...
    explicit TaskExecutionHostV2(TaskCoordinator* coordinator, ArtifactStoreV2* artifactStore = nullptr);
    ...
    AdapterEventEndpointV2 adapterEndpoint() const;

private:
    void consumeAdapterEvent(const ProtocolEnvelope& event);
    void finishAdapter(const PythonAdapterExitV2& outcome);
    ...
    bool startAdapter(const PythonAdapterLaunchV2& launch, QString* error);
    ...
    PythonAdapterHostV2 adapterHost_;
