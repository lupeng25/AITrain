...
inline constexpr int kEvidenceBundleSchemaVersion = 2;
...
struct EvidenceExternalInput final {
    QString role;
...
struct EvidenceBundle final {
    int schemaVersion = kEvidenceBundleSchemaVersion;
    ...
    QVector<EvidenceExternalInput> externalInputs;
    QVector<EvidenceArtifact> artifacts;
    QStringList limitations;
    QDateTime createdAt;
};

bool validateEvidenceBundle(const EvidenceBundle& bundle, QString* error = nullptr);
QJsonObject encodeEvidenceBundle(const EvidenceBundle& bundle, QString* error = nullptr);
bool decodeEvidenceBundle(const QJsonObject& object, EvidenceBundle* bundle, QString* error = nullptr);
