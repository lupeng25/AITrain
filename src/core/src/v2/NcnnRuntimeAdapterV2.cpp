...
QString binPath(const RuntimeModelLocationV2& model)
{
...
QJsonObject runtimeOptions(const RuntimeModelLocationV2& model)
{
    const TensorContractV2& input = model.manifest.inputs.constFirst();
    QJsonArray outputs;
    for (const TensorContractV2& output : model.manifest.outputs) outputs.append(output.name);
    QJsonObject options{
...
bool writeJson(const QString& path, const QJsonObject& object, QString* error)
{
    ...
    if (!file.open(QIODevice::WriteOnly)
        ...
        || !file.commit()) {
        if (error) *error = QStringLiteral("无法写入 V2 NCNN 推理预测结果：%1").arg(path);
        return false;
    }
    return true;
}
...
bool writeOverlay(const QString& path, const QImage& overlay, QString* error)
{
    ...
    if (!file.open(QIODevice::WriteOnly) || !overlay.save(&file, "PNG") || !file.commit()) {
        if (error) *error = QStringLiteral("无法写入 V2 NCNN 推理 overlay：%1").arg(path);
        return false;
    }
    return true;
}
...
QString NcnnRuntimeAdapterV2::runtimeRoute() const
{
    return QStringLiteral("aitrain_ncnn");
}

RuntimeOperationResultV2 NcnnRuntimeAdapterV2::validateModel(const RuntimeModelLocationV2& model) const
{
    ...
    if (model.manifest.artifactFormat != QStringLiteral("ncnn")
        ...
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            QStringLiteral("NCNN V2 只接受 artifactFormat=ncnn 且入口为 .param 的模型包。"));
    }
    if (!supportedContract(model.manifest)) {
        return failed(RuntimeStatusV2::RuntimeNotImplemented,
            QStringLiteral("NCNN V2 未实现 Manifest 声明的 modelFamily/decoder：%1/%2。")
    ...
    if (model.manifest.inputs.size() != 1
        ...
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            QStringLiteral("NCNN V2 要求 Manifest 明确一个静态 NCHW 输入及 blob name。"));
    }
    ...
    if (model.manifest.outputs.size() < minimumOutputs) {
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            QStringLiteral("NCNN V2 Manifest 的输出 blob 合同不完整。"));
    }
    QSet<QString> blobNames{model.manifest.inputs.constFirst().name};
    for (const TensorContractV2& output : model.manifest.outputs) {
        if (output.name.trimmed().isEmpty() || blobNames.contains(output.name)) {
            return failed(RuntimeStatusV2::ArtifactIncompatible,
                QStringLiteral("NCNN V2 Manifest 的 input/output blob name 必须非空且唯一。"));
        }
    ...
    for (const TensorContractV2& output : model.manifest.outputs) outputBlobs.append(output.name);
...
RuntimeOperationResultV2 NcnnRuntimeAdapterV2::probe(const RuntimeModelLocationV2& model) const
{
    RuntimeOperationResultV2 result = validateModel(model);
    if (result.status != RuntimeStatusV2::Available) return result;
    const aitrain::NcnnBackendStatus backend = aitrain::ncnnBackendStatus();
    if (!backend.sdkAvailable) return failed(RuntimeStatusV2::SdkMissing, backend.message);
    if (!backend.inferenceAvailable) return failed(RuntimeStatusV2::RuntimeNotImplemented, backend.message);
    result.details.insert(QStringLiteral("backend"), backend.toJson());
    return result;
}

RuntimeOperationResultV2 NcnnRuntimeAdapterV2::infer(const RuntimeModelLocationV2& model, const QJsonObject& request) const
{
    ...
    if (imagePath.isEmpty() || outputPath.isEmpty()) {
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            QStringLiteral("NCNN V2 推理请求缺少 imagePath 或 outputPath。"));
    }
    if (!QDir().mkpath(outputPath)) {
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            QStringLiteral("无法创建 V2 NCNN 推理输出目录：%1").arg(outputPath));
    }
    const QString paramPath = QDir(model.artifactDirectory).filePath(model.manifest.artifactEntryPath);
    ...
    if (!error.isEmpty() || overlay.isNull()) {
        return failed(RuntimeStatusV2::ArtifactIncompatible,
            error.isEmpty() ? QStringLiteral("V2 NCNN 推理未生成可用 overlay。") : error);
    }
...
RuntimeOperationResultV2 NcnnRuntimeAdapterV2::benchmark(const RuntimeModelLocationV2& model, const QJsonObject& request) const
{
    QElapsedTimer timer;
    timer.start();
...
RuntimeOperationResultV2 NcnnRuntimeAdapterV2::deploymentValidate(const RuntimeModelLocationV2& model, const QJsonObject& request) const
{
