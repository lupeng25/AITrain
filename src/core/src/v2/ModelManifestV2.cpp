...
bool validateTensor(const TensorContractV2& tensor, const QString& role, QString* error)
{
...
QJsonArray encodeTensors(const QVector<TensorContractV2>& tensors)
{
    QJsonArray values;
    for (const TensorContractV2& tensor : tensors) {
        QJsonArray shape;
...
bool decodeTensors(const QJsonValue& value, QVector<TensorContractV2>* tensors, const QString& role, QString* error)
{
    if (!value.isArray()) {
...
bool validateModelManifestV2(const ModelManifestV2& manifest, QString* error)
{
    ...
    if (!manifest.modelPackageId.isValid() || !manifest.sourceTaskId.isValid() || !manifest.sourceSnapshotId.isValid()
        ...
        if (error) *error = QStringLiteral("Model Manifest V2 缺少必需字段、未验证、来源哈希或产物入口路径无效。");
        return false;
    }
    ...
    for (const TensorContractV2& tensor : manifest.inputs) if (!validateTensor(tensor, QStringLiteral("input"), error)) return false;
    for (const TensorContractV2& tensor : manifest.outputs) if (!validateTensor(tensor, QStringLiteral("output"), error)) return false;
    for (const QString& route : manifest.runtimeRoutes) {
        if (route.trimmed().isEmpty()) {
            if (error) *error = QStringLiteral("Model Manifest V2 runtime route 不能为空。");
            return false;
        }
    }
    return true;
...
QJsonObject encodeModelManifestV2(const ModelManifestV2& manifest, QString* error)
{
    if (!validateModelManifestV2(manifest, error)) return {};
    return QJsonObject{{QStringLiteral("schemaVersion"), kModelManifestV2SchemaVersion},
...
bool decodeModelManifestV2(const QJsonObject& object, ModelManifestV2* manifest, QString* error)
{
    if (object.value(QStringLiteral("schemaVersion")).toInt(-1) != kModelManifestV2SchemaVersion) {
        if (error) *error = QStringLiteral("不支持的 Model Manifest schemaVersion。");
        return false;
    }
    ModelManifestV2 parsed;
    ...
    if (!inputsDecoded || !outputsDecoded
        ...
        if (error && error->isEmpty()) *error = QStringLiteral("Model Manifest V2 JSON 字段类型无效。");
        return false;
    }
    ...
    if (!validateModelManifestV2(parsed, error)) return false;
    if (manifest) *manifest = parsed;
    return true;
}

bool decodeModelManifestImportDraftV2(const QJsonObject& object, ModelManifestV2* manifest, QString* error)
{
    ...
    return decodeModelManifestV2(normalized, manifest, error);
}

ModelManifestStatusV2 modelManifestStatusV2(const QJsonObject* object, QString* error)
{
    if (!object || object->isEmpty()) return ModelManifestStatusV2::Unclassified;
    ModelManifestV2 manifest;
    return decodeModelManifestV2(*object, &manifest, error) ? ModelManifestStatusV2::Valid : ModelManifestStatusV2::Invalid;
}

bool canUseModelManifestV2ForRuntime(const QJsonObject* object, const QString& runtimeRoute, QString* error)
{
    ModelManifestV2 manifest;
    if (modelManifestStatusV2(object, error) != ModelManifestStatusV2::Valid || !decodeModelManifestV2(*object, &manifest, error)) return false;
