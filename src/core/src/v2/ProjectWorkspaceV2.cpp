...
bool verifyArtifactFile(const QString& artifactPath,
    ...
    VerifiedWorkflowArtifactFileV2* result,
    QString* error)
{
...
const VerifiedWorkflowArtifactFileV2* selectArtifactFile(const VerifiedTrainingWorkflowInputV2& input,
    const QStringList& preferredRelativePaths)
{
    for (const QString& relativePath : preferredRelativePaths) {
        for (const VerifiedWorkflowArtifactFileV2& file : input.files) {
...
const VerifiedWorkflowArtifactFileV2* selectDeploymentSample(
    const VerifiedTrainingWorkflowInputV2& snapshotInput,
    ...
    QString* error)
{
    ...
    if (!requestedRelativePath.trimmed().isEmpty()) {
        ...
        const auto found = std::find_if(snapshotInput.files.cbegin(), snapshotInput.files.cend(),
            [&requested](const VerifiedWorkflowArtifactFileV2& file) {
    ...
    QVector<const VerifiedWorkflowArtifactFileV2*> candidates;
    for (const VerifiedWorkflowArtifactFileV2& file : snapshotInput.files) {
...
const TrainingWorkflowStepProfileV2* workflowStepProfile(const TrainingWorkflowProfileV2& profile,
    const QString& kind)
{
    for (const TrainingWorkflowStepProfileV2& step : profile.steps) {
        if (step.kind == kind) return &step;
    }
    return nullptr;
}

bool datasetSnapshotForWorkflow(const StorageV2& storage,
    ...
    QString* error)
{
    ...
    const auto snapshotStep = std::find_if(steps.cbegin(), steps.cend(), [](const WorkflowStepSnapshotV2& step) {
        return step.kind == QStringLiteral("CreateSnapshot")
            && step.state == WorkflowStepState::Succeeded
            && step.outputArtifactId.isValid();
    });
    WorkflowInputBindingV2 binding;
...
bool projectedTerminalTaskForWorkflow(const TaskSnapshot& persisted,
    const WorkflowTerminalizationSnapshotV2& terminalization,
    TaskSnapshot* result,
    QString* error)
{
    if (!result || terminalization.taskId != persisted.id
        || terminalization.state == WorkflowTerminalizationStateV2::Closed) {
...
TaskState taskStateForWorkflowResult(const WorkflowRunExecutionResultV2& result)
{
...
bool resolveVerifiedArtifact(const StorageV2& storage,
    const ArtifactStoreV2* artifactStore,
...
bool parseTensorContracts(const QJsonValue& value, QVector<TensorContractV2>* result, QString* error)
{
    if (!result || !value.isArray()) {
        if (error) *error = QStringLiteral("官方导出 sidecar 的张量合同必须为数组。");
        return false;
    }
    QVector<TensorContractV2> tensors;
...
bool modelManifestFromTrainingExportSidecar(const QJsonObject& sidecar,
    const TrainingWorkflowProfileV2& profile,
    const TaskId& taskId,
    const SnapshotId& snapshotId,
    const VerifiedWorkflowArtifactFileV2& exportedModel,
    ModelManifestV2* result,
    QString* error)
{
    ...
    if (!parseTensorContracts(contract.value(QStringLiteral("inputs")), &manifest.inputs, error)
        ...
        if (error && error->isEmpty()) *error = QStringLiteral("官方导出 sidecar 缺少 V2 Model Manifest 合同。");
        return false;
    }
    ...
    if (!validateModelManifestV2(manifest, error)) return false;
    *result = manifest;
    return true;
}
...
bool modelManifestFromAnomalibBundleSidecar(const QJsonObject& sidecar,
    ...
    const TaskId& taskId,
    const SnapshotId& snapshotId,
    const VerifiedWorkflowArtifactFileV2& sidecarFile,
    ModelManifestV2* result,
    QString* error)
{
    ...
    manifest.verified = true;
    manifest.limitations = profile.limitations;
    if (manifest.sourceBackend != profile.trainingBackend || !validateModelManifestV2(manifest, error)) return false;
    *result = manifest;
    return true;
}
...
bool modelManifestFromPaddleOcrBundleSidecar(const QJsonObject& sidecar,
    ...
    const SnapshotId& snapshotId,
    const VerifiedWorkflowArtifactFileV2& sidecarFile,
    const VerifiedWorkflowArtifactFileV2& bundleFile,
    ModelManifestV2* result,
    QString* error)
{
...
QJsonObject artifactFacts(const ArtifactSnapshotV2& artifact)
{
    QJsonArray files;
    for (const ArtifactFileSnapshot& file : artifact.files) {
...
WorkflowStepExecutionResultV2 workflowExecutionForAdapterTerminal(const ProtocolEnvelope& event,
    const ArtifactId& outputArtifactId)
{
    WorkflowStepExecutionResultV2 execution;
...
ProjectWorkspaceV2::ProjectWorkspaceV2() = default;

bool ProjectWorkspaceV2::recoverEvidenceGatedWorkflows(QString* error)
{
    const QVector<WorkflowRunSnapshotV2> workflows = storage_.pendingEvidenceRequiredWorkflows(1000, error);
    if (error && !error->isEmpty()) return false;
    const QVector<WorkflowTerminalizationSnapshotV2> pending = storage_.pendingWorkflowTerminalizations(1000, error);
    if (error && !error->isEmpty()) return false;
    QHash<QString, WorkflowTerminalizationSnapshotV2> terminalizations;
    for (const WorkflowTerminalizationSnapshotV2& terminalization : pending) {
        terminalizations.insert(terminalization.workflowRunId.toString(), terminalization);
    }

    for (const WorkflowRunSnapshotV2& workflow : workflows) {
        TaskSnapshot task;
        if (!storage_.task(workflow.taskId, &task, error)) return false;
        WorkflowTerminalizationSnapshotV2 terminalization = terminalizations.value(workflow.id.toString());
        if (!terminalization.workflowRunId.isValid()) {
            WorkflowRunnerV2 runner(&storage_);
            WorkflowStepDispatchV2 dispatch;
            const QVector<WorkflowStepSnapshotV2> steps = storage_.workflowSteps(workflow.id, error);
            if (error && !error->isEmpty()) return false;
            const auto running = std::find_if(steps.cbegin(), steps.cend(), [](const WorkflowStepSnapshotV2& step) {
                return step.state == WorkflowStepState::Running;
            });
            if (running != steps.cend()) {
                WorkflowStepExecutionResultV2 interrupted;
        ...
        if (terminalization.state == WorkflowTerminalizationStateV2::Sealed) {
            EvidenceBundleV2 evidence;
            EvidenceArtifactBundleV2 committed;
...
bool ProjectWorkspaceV2::open(const QString& projectRoot, QString* error)
{
    close();
    const QString normalizedRoot = QDir::cleanPath(QDir::fromNativeSeparators(projectRoot.trimmed()));
    if (normalizedRoot.isEmpty()) {
        if (error) *error = QStringLiteral("打开 V2 项目工作区需要项目根目录。");
        return false;
    }
    const QFileInfo rootInfo(normalizedRoot);
    if (rootInfo.exists() && (!rootInfo.isDir() || rootInfo.isSymLink())) {
        if (error) *error = QStringLiteral("V2 项目根目录必须是非符号链接目录。");
        return false;
    }
    const QString candidate = QDir(normalizedRoot).filePath(QStringLiteral(".aitrain-v2"));
    if (!QDir().mkpath(candidate)) {
        if (error) *error = QStringLiteral("无法创建 V2 项目工作区：%1").arg(candidate);
        return false;
    }
    if (!storage_.open(QDir(candidate).filePath(QStringLiteral("project-v2.sqlite")), error)) {
        close();
        return false;
    }
    ...
    trainingAdapterHost_ = std::make_unique<TaskExecutionHostV2>(taskCoordinator_.get(), artifactStore_.get());
    return true;
}

void ProjectWorkspaceV2::close()
{
...
bool ProjectWorkspaceV2::isOpen() const
{
    return storage_.isOpen() && artifactStore_ != nullptr;
}

bool ProjectWorkspaceV2::importModel(const ModelImportRequestV2& request,
    ModelImportResultV2* result,
    QString* error,
    const aitrain::CancellationCallback& cancellation)
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return false;
    }
    TaskCoordinator coordinator(&storage_);
    ModelImportServiceV2 importer(&coordinator, artifactStore_.get());
    return importer.importModel(request, result, error, cancellation);
}

bool ProjectWorkspaceV2::startTask(const TaskId& taskId,
...
bool ProjectWorkspaceV2::requestTaskCancellation(const TaskId& taskId, QString* error)
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return false;
    ...
    if (!taskCoordinator_) {
        if (error) *error = QStringLiteral("V2 项目任务协调器不可用。");
        return false;
    }
...
bool ProjectWorkspaceV2::finalizeTask(const TaskId& taskId,
...
bool ProjectWorkspaceV2::commitDatasetSnapshot(const TaskId& taskId,
    ...
    QString* error)
{
    ...
    if (!artifactStore_->begin(taskId, QStringLiteral("dataset_snapshot_v2"), &artifactId, &artifactStaging, error)) {
        return false;
    }
    const auto abort = [&]() {
        QString ignored;
    ...
    if (!copyDatasetTreeToEmptyStaging(request.datasetRoot, artifactStaging, cancellation, error)
        || !createDatasetSnapshotV2(artifactStaging, stagingManifestPath, request.datasetFormat.trimmed(),
            request.driverId.trimmed(), request.driverVersion.trimmed(), request.options, &snapshotResult, error)) {
        abort();
        return false;
    }
    QString artifactPath;
    if (!artifactStore_->commit(artifactId, taskId, QStringLiteral("dataset_snapshot_v2"), artifactStaging,
...
bool ProjectWorkspaceV2::beginTrainingWorkflow(const TaskId& taskId,
    ...
    QString* error)
{
    ...
    TrainingWorkflowProfileV2 profile;
    if (!resolveTrainingWorkflowProfileV2(request.trainingBackend, &profile, error)
    ...
    VerifiedTrainingWorkflowInputV2 verifiedSnapshot;
    ...
    const VerifiedWorkflowArtifactFileV2* verifiedManifest = selectArtifactFile(
    ...
    workflow.terminalPolicy = request.requireEvidenceBeforeTerminal
        ? WorkflowTerminalPolicyV2::EvidenceRequired
        : WorkflowTerminalPolicyV2::Immediate;
    ...
    const QJsonObject validationReport{
        ...
        {QStringLiteral("validator"), QStringLiteral("snapshot_integrity_v2")},
    ...
    RuntimeArtifactBundleV2 validationBundle;
    if (!QDir().mkpath(validationRoot)
        || !writeJsonFile(validationPath, validationReport, error)
        || !commitRuntimeArtifacts(taskId, QStringLiteral("dataset_validation_v2"),
    ...
    for (int ordinal = 0; ordinal < profile.steps.size(); ++ordinal) {
        const TrainingWorkflowStepProfileV2& stepProfile = profile.steps.at(ordinal);
    ...
    WorkflowRunnerV2 runner(&storage_);
    TrainingWorkflowDispatchV2 dispatch;
    ...
    WorkflowStepExecutionResultV2 preflight;
...
bool ProjectWorkspaceV2::completeTrainingWorkflowStep(const WorkflowRunId& workflowRunId,
    ...
    QString* error)
{
    ...
    if (!dispatch.dispatch.hasStep && isTerminalWorkflowStepState(dispatch.dispatch.result.state)) {
        ...
        if (workflow.terminalPolicy == WorkflowTerminalPolicyV2::EvidenceRequired) {
...
bool ProjectWorkspaceV2::startTrainingWorkflowAdapterStep(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    ...
    TrainingWorkflowAdapterEventHandlerV2 eventHandler)
{
    ...
    const auto stepIt = std::find_if(steps.cbegin(), steps.cend(), [&workflowStepId](const WorkflowStepSnapshotV2& step) {
    ...
    auto completedDispatch = std::make_shared<TrainingWorkflowDispatchV2>();
    ...
    return trainingAdapterHost_->startExistingTask(task, launch,
        [this, workflowRunId, workflowStepId, completedDispatch, completed](const ProtocolEnvelope& terminal,
            ...
            const WorkflowStepExecutionResultV2 execution = workflowExecutionForAdapterTerminal(terminal, outputArtifactId);
...
bool ProjectWorkspaceV2::requestTrainingWorkflowAdapterCancellation(const TaskId& taskId, QString* error)
{
...
bool ProjectWorkspaceV2::isTrainingWorkflowAdapterRunning() const
{
    return trainingAdapterHost_ && trainingAdapterHost_->isRunning();
}

AdapterEventEndpointV2 ProjectWorkspaceV2::trainingWorkflowAdapterEndpoint() const
{
    return trainingAdapterHost_ ? trainingAdapterHost_->adapterEndpoint() : AdapterEventEndpointV2{};
}

bool ProjectWorkspaceV2::resolveTrainingWorkflowStepInput(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    VerifiedTrainingWorkflowInputV2* result,
    QString* error) const
{
    ...
    VerifiedTrainingWorkflowInputV2 resolved;
    resolved.artifactId = artifact.id;
    resolved.artifactPath = artifactPath;
    for (const ArtifactFileSnapshot& file : artifact.files) {
        VerifiedWorkflowArtifactFileV2 verified;
...
bool ProjectWorkspaceV2::prepareTrainingWorkflowAdapterLaunch(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    ...
    QString* error) const
{
    ...
    TrainingWorkflowProfileV2 profile;
    if (!resolveTrainingWorkflowProfileV2(trainingBackend, &profile, error)
        || workflow.templateId != profile.templateId) {
        if (error && error->isEmpty()) *error = QStringLiteral("训练 Workflow 缺少匹配的已注册 Profile。");
        return false;
    }
    const TrainingWorkflowStepProfileV2* route = workflowStepProfile(profile, stepIt->kind);
    ...
    VerifiedTrainingWorkflowInputV2 snapshotInput;
    ...
    VerifiedTrainingWorkflowInputV2 input;
    if (!resolveTrainingWorkflowStepInput(workflowRunId, workflowStepId, &input, error)) return false;
    const VerifiedWorkflowArtifactFileV2* snapshotManifest = selectArtifactFile(input, {QStringLiteral("dataset_snapshot.json")});
    ...
    if (stepIt->kind == QStringLiteral("Train")) {
        ...
        const VerifiedWorkflowArtifactFileV2* modelInput = selectArtifactFile(input, route->artifactCandidates);
        if (!modelInput) {
        ...
        const VerifiedWorkflowArtifactFileV2* sidecar = selectArtifactFile(input,
        ...
        const VerifiedWorkflowArtifactFileV2* checkpoint = selectArtifactFile(input,
        ...
        const VerifiedWorkflowArtifactFileV2* configFile = selectArtifactFile(input,
        ...
        const VerifiedWorkflowArtifactFileV2* dictionary = selectArtifactFile(input,
        ...
        const VerifiedWorkflowArtifactFileV2* sidecar = selectArtifactFile(input,
            {QStringLiteral("model_sidecar/semantic_segmentation_sidecar.json")});
        if (sidecar) request.insert(QStringLiteral("sidecarPath"), sidecar->absolutePath);
        const VerifiedWorkflowArtifactFileV2* evaluationReport = selectArtifactFile(input,
        ...
        if (profile.artifactFormat == QStringLiteral("anomalib_bundle")) {
            const VerifiedWorkflowArtifactFileV2* anomalySidecar = selectArtifactFile(input,
                {QStringLiteral("anomaly_sidecar/anomaly_sidecar.json")});
            const VerifiedWorkflowArtifactFileV2* anomalyCheckpoint = selectArtifactFile(input,
                {QStringLiteral("checkpoint/model.ckpt")});
        ...
        const VerifiedWorkflowArtifactFileV2* bundle = selectArtifactFile(input,
            {route->artifactCandidates.value(1)});
        const VerifiedWorkflowArtifactFileV2* sample = selectDeploymentSample(
...
bool ProjectWorkspaceV2::registerTrainingWorkflowModel(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    ...
    QString* error)
{
    ...
    if (!storage_.workflowRun(workflowRunId, &workflow, error)) return false;
    const QVector<WorkflowStepSnapshotV2> steps = storage_.workflowSteps(workflowRunId, error);
    const auto registerIt = std::find_if(steps.cbegin(), steps.cend(), [&workflowStepId](const WorkflowStepSnapshotV2& step) {
        return step.id == workflowStepId;
    });
    ...
    const TrainingWorkflowStepProfileV2* registerRoute = workflowStepProfile(profile, QStringLiteral("RegisterModel"));
    if (!registerRoute || registerRoute->artifactCandidates.size() < 2) {
        if (error) *error = QStringLiteral("训练 Profile 缺少 RegisterModel 合同。");
        return false;
    }
    const auto exportIt = std::find_if(steps.cbegin(), steps.cend(), [](const WorkflowStepSnapshotV2& step) {
    ...
    VerifiedTrainingWorkflowInputV2 exported;
    if (!resolveVerifiedArtifact(storage_, artifactStore_.get(), exportIt->outputArtifactId, &exported, error)) return false;
    const VerifiedWorkflowArtifactFileV2* entry = selectArtifactFile(exported,
        {registerRoute->artifactCandidates.value(0)});
    const VerifiedWorkflowArtifactFileV2* companion = selectArtifactFile(exported,
    ...
    const VerifiedWorkflowArtifactFileV2* sidecar = (anomalibBundle || paddleOcrBundle) ? entry : companion;
    ...
    if (!artifactStore_->begin(workflow.taskId, QStringLiteral("model_registration_v2"), &registrationArtifactId, &registrationStagingPath, error)) return false;
    const QString manifestPath = QDir(registrationStagingPath).filePath(QStringLiteral("model_manifest_v2.json"));
    const QJsonObject encoded = encodeModelManifestV2(manifest, error);
    ...
    if (!artifactStore_->commit(registrationArtifactId, workflow.taskId, QStringLiteral("model_registration_v2"), registrationStagingPath,
            &storage_, &registrationArtifactPath, error)) {
        QString ignored;
        artifactStore_->abort(registrationStagingPath, &ignored);
        return false;
    }
    ModelPackageSnapshotV2 package{manifest, exportIt->outputArtifactId, QDateTime::currentDateTimeUtc()};
    ...
    result->registrationArtifact.pathsByKind.insert(QStringLiteral("model_manifest_v2"),
        QDir(registrationArtifactPath).filePath(QStringLiteral("model_manifest_v2.json")));
    return true;
}

bool ProjectWorkspaceV2::prepareTrainingWorkflowDeploymentInvocation(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    ...
    QString* error) const
{
    ...
    if (!storage_.workflowRun(workflowRunId, &workflow, error)) return false;
    const QVector<WorkflowStepSnapshotV2> steps = storage_.workflowSteps(workflowRunId, error);
    const auto deploymentIt = std::find_if(steps.cbegin(), steps.cend(), [&workflowStepId](const WorkflowStepSnapshotV2& step) {
        return step.id == workflowStepId;
    });
    ...
    if (!resolveTrainingWorkflowProfileV2(trainingBackend, &profile, error)
        || workflow.templateId != profile.templateId) return false;
    const TrainingWorkflowStepProfileV2* deploymentRoute = workflowStepProfile(profile, QStringLiteral("DeploymentValidate"));
    ...
    VerifiedTrainingWorkflowInputV2 exported;
    if (!resolveVerifiedArtifact(storage_, artifactStore_.get(), exportIt->outputArtifactId, &exported, error)) return false;
    const VerifiedWorkflowArtifactFileV2* model = selectArtifactFile(exported,
        {deploymentRoute->artifactCandidates.at(0)});
    const VerifiedWorkflowArtifactFileV2* sidecar = selectArtifactFile(exported,
    ...
    RuntimeInvocationV2 invocation;
    ...
    const QJsonObject encoded = encodeRuntimeInvocationV2(invocation, error);
...
bool ProjectWorkspaceV2::renderTrainingWorkflowDeliveryReport(const WorkflowRunId& workflowRunId,
    const WorkflowStepId& workflowStepId,
    ...
    QString* error)
{
    ...
    if (!storage_.workflowRun(workflowRunId, &workflow, error)) return false;
    const QVector<WorkflowStepSnapshotV2> steps = storage_.workflowSteps(workflowRunId, error);
    const auto renderIt = std::find_if(steps.cbegin(), steps.cend(), [&workflowStepId](const WorkflowStepSnapshotV2& step) {
        return step.id == workflowStepId;
    });
    ...
    DatasetSnapshotRecordV2 snapshot;
    WorkflowInputBindingV2 externalInput;
    ...
    const QJsonObject report{{QStringLiteral("schemaVersion"), 3}, {QStringLiteral("kind"), QStringLiteral("training_delivery_report_v2")},
    ...
    markdownLines << QStringLiteral("# AITrain V2 训练交付摘要")
    ...
    ArtifactId artifactId;
    QString stagingPath;
    if (!artifactStore_->begin(workflow.taskId, QStringLiteral("training_delivery_report_v2"), &artifactId, &stagingPath, error)) return false;
    ...
    QString artifactPath;
    if (!artifactStore_->commit(artifactId, workflow.taskId, QStringLiteral("training_delivery_report_v2"), stagingPath,
...
bool ProjectWorkspaceV2::commitRuntimeArtifacts(const TaskId& taskId,
    ...
    QString* error)
{
    ...
    for (const RuntimeArtifactCandidateV2& candidate : candidates) {
...
bool ProjectWorkspaceV2::buildWorkflowEvidenceBundle(const WorkflowRunId& workflowRunId,
    ...
    QString* error) const
{
    ...
    const QVector<MetricSnapshotV2> metrics = storage_.metricsForTask(task.id, error);
    if (error && !error->isEmpty()) return false;

    EvidenceBundleV2 bundle;
    ...
    const bool hasSucceededSnapshotStep = std::any_of(steps.cbegin(), steps.cend(), [](const WorkflowStepSnapshotV2& step) {
    ...
    if (hasSucceededSnapshotStep) {
        ...
        bundle.externalInputs.append(EvidenceExternalInputV2{
        ...
        for (const WorkflowStepSnapshotV2& step : steps) {
            ...
            if (storage_.workflowInput(workflow.id, QStringLiteral("dataset_snapshot"),
                ...
                bundle.externalInputs.append(EvidenceExternalInputV2{
                    ...
                    binding.datasetVersionId, binding.datasetSnapshotId, binding.manifestSha256,
                    binding.rootHash});
            } else if (workflow.templateId == QStringLiteral("dataset_quality_v2")) {
                if (error) {
    ...
    const auto appendArtifact = [&](const ArtifactId& artifactId, const QString& role,
                                    const WorkflowStepSnapshotV2* step) -> bool {
        ...
        if (index < 0) {
            ...
            EvidenceArtifactV2 evidenceArtifact;
    ...
    const QVector<ArtifactSnapshotV2> taskArtifacts = storage_.artifactsForTask(task.id, error);
    if (error && !error->isEmpty()) return false;
    for (const ArtifactSnapshotV2& artifact : taskArtifacts) {
    ...
    for (const MetricSnapshotV2& metric : metrics) {
    ...
    bundle.backendEnvironment = {{QStringLiteral("backends"), backendList},
        {QStringLiteral("evidenceSource"), QStringLiteral("aitrain_v2_persisted_storage")}};
    ...
    const auto trainStep = std::find_if(steps.cbegin(), steps.cend(), [](const WorkflowStepSnapshotV2& step) {
        return step.kind == QStringLiteral("Train");
    });
    TrainingWorkflowProfileV2 profile;
    if (trainStep != steps.cend() && resolveTrainingWorkflowProfileV2(trainStep->backend, &profile, nullptr)) {
    ...
    if (!validateEvidenceBundleV2(bundle, error)) return false;
    *result = bundle;
    return true;
}

bool ProjectWorkspaceV2::commitEvidenceBundle(const EvidenceBundleV2& bundle,
    EvidenceArtifactBundleV2* result,
    QString* error)
{
    if (!isOpen() || !result || !validateEvidenceBundleV2(bundle, error)) {
    ...
    const QByteArray json = EvidenceRendererV2::renderJson(bundle, error);
    if (json.isEmpty()) return false;
    const QString markdown = EvidenceRendererV2::renderMarkdown(bundle, error);
    if (markdown.isEmpty()) return false;
    const QString html = EvidenceRendererV2::renderHtml(bundle, error);
    if (html.isEmpty()) return false;
    const QByteArray modelCard = EvidenceRendererV2::renderModelCard(bundle, error);
    if (modelCard.isEmpty()) return false;

    ArtifactId artifactId;
    QString stagingPath;
    if (!artifactStore_->begin(bundle.task.id, QStringLiteral("evidence_bundle_v2"), &artifactId, &stagingPath, error)) return false;
    const auto abort = [&]() {
        QString ignored;
    ...
    QString artifactPath;
    const WorkflowRunId terminalizationWorkflow = workflow.terminalPolicy == WorkflowTerminalPolicyV2::EvidenceRequired
        ? bundle.workflowRunId : WorkflowRunId{};
    if (!artifactStore_->commit(artifactId, bundle.task.id, QStringLiteral("evidence_bundle_v2"),
...
bool ProjectWorkspaceV2::recordWorkflowEvidenceFailure(const WorkflowRunId& workflowRunId,
    const Failure& failure,
    QString* error)
{
...
bool ProjectWorkspaceV2::closeWorkflowTerminalization(const WorkflowRunId& workflowRunId,
    QString* error)
{
    if (!isOpen() || !workflowRunId.isValid()) {
...
QString ProjectWorkspaceV2::runtimeStagingPath(const TaskId& taskId) const
{
...
bool ProjectWorkspaceV2::cleanupRuntimeStaging(const TaskId& taskId, QString* error)
{
...
QVector<TaskSnapshot> ProjectWorkspaceV2::tasks(int limit, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
QVector<DatasetCatalogItemV2> ProjectWorkspaceV2::datasets(int limit, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
bool ProjectWorkspaceV2::task(const TaskId& taskId, TaskSnapshot* result, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return false;
...
QVector<ArtifactSnapshotV2> ProjectWorkspaceV2::artifactsForTask(const TaskId& taskId, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
QVector<MetricSnapshotV2> ProjectWorkspaceV2::metricsForTask(const TaskId& taskId, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
QVector<WorkflowRunSnapshotV2> ProjectWorkspaceV2::workflowRunsForTask(const TaskId& taskId, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
QVector<WorkflowStepSnapshotV2> ProjectWorkspaceV2::workflowSteps(const WorkflowRunId& workflowRunId, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
QVector<ModelPackageSnapshotV2> ProjectWorkspaceV2::modelPackages(int limit, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return {};
...
bool ProjectWorkspaceV2::projectSummary(ProjectSummarySnapshotV2* result, QString* error) const
{
    if (!isOpen()) {
        if (error) *error = QStringLiteral("V2 项目工作区未打开。");
        return false;
...
QString ProjectWorkspaceV2::workspacePath() const
{
