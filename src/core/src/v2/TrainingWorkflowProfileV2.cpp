...
TrainingWorkflowStepProfileV2 step(const QString& kind,
...
TrainingWorkflowProfileV2 yoloProfile(const QString& trainingBackend,
...
{
    TrainingWorkflowProfileV2 value;
    value.trainingBackend = trainingBackend;
    value.templateId = QStringLiteral("official_yolo_training_delivery_v2");
    ...
    value.steps = {
        step(QStringLiteral("ValidateDataset"), QStringLiteral("dataset_driver_v2")),
        step(QStringLiteral("CreateSnapshot"), QStringLiteral("snapshot_store_v2")),
        ...
        step(QStringLiteral("RegisterModel"), QStringLiteral("storage_v2"), {},
            {QStringLiteral("export/model.onnx"),
                QStringLiteral("export_sidecar/model.aitrain-export.json")}),
        step(QStringLiteral("RenderDeliveryReport"), QStringLiteral("evidence_renderer_v2"))};
    return value;
}

TrainingWorkflowProfileV2 smpProfile()
{
    TrainingWorkflowProfileV2 value;
    value.trainingBackend = QStringLiteral("smp_semantic_segmentation");
    value.templateId = QStringLiteral("smp_semantic_segmentation_delivery_v2");
...
TrainingWorkflowProfileV2 anomalibProfile(const QString& trainingBackend, const QStringList& extraLimitations)
{
    TrainingWorkflowProfileV2 value;
    value.trainingBackend = trainingBackend;
    value.templateId = QStringLiteral("anomalib_training_delivery_v2");
    ...
    value.exportBackend = QStringLiteral("anomalib_artifact_export_v2");
...
TrainingWorkflowProfileV2 paddleOcrProfile(bool recognition)
{
    TrainingWorkflowProfileV2 value;
    ...
    value.templateId = recognition
        ? QStringLiteral("paddleocr_rec_training_delivery_v2")
        : QStringLiteral("paddleocr_det_training_delivery_v2");
...
{
    static const QVector<TrainingWorkflowProfileV2> profiles{
        yoloProfile(QStringLiteral("ultralytics_yolo_detect"),
            ...
            {QStringLiteral("当前 V2 训练交付模型包仅声明已验证的 AITrain ONNX Runtime 路线。"),
                QStringLiteral("公开或生成数据集的工作流证据不代表客户域精度。")}),
    ...
    QString* error)
{
    ...
    for (const TrainingWorkflowProfileV2& candidate : trainingWorkflowProfilesV2()) {
...
{
    TrainingWorkflowProfileV2 ignored;
    return resolveTrainingWorkflowProfileV2(trainingBackend, &ignored, nullptr);
}
