...
Failure diagnosticsFailure(FailureCode code, const QString& message)
{
    Failure failure;
    failure.code = code;
    failure.message = message.isEmpty() ? QStringLiteral("diagnostics_v2_failed") : message;
    failure.suggestedAction = code == FailureCode::Canceled
        ? QStringLiteral("如需继续，请重新发起 Diagnostics Bundle V2。")
...
bool writeFile(const QString& path, const QByteArray& bytes, QString* error)
{
    if (!QDir().mkpath(QFileInfo(path).absolutePath())) {
        if (error) *error = QStringLiteral("diagnostics_v2_directory_create_failed");
        return false;
    }
    QSaveFile file(path);
    if (!file.open(QIODevice::WriteOnly) || file.write(bytes) != bytes.size() || !file.commit()) {
        if (error) *error = QStringLiteral("diagnostics_v2_file_write_failed:%1").arg(file.errorString());
        return false;
    }
    return true;
}
...
bool hashFile(const QString& path, QString* hash, QString* error)
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        if (error) *error = QStringLiteral("diagnostics_v2_artifact_unreadable");
        return false;
    }
    ...
    while (!file.atEnd()) {
        ...
        if (bytes.isEmpty() && file.error() != QFileDevice::NoError) {
            if (error) *error = QStringLiteral("diagnostics_v2_artifact_read_failed");
            return false;
        }
...
bool verifyArtifact(const ArtifactSnapshotV2& artifact, const QString& root,
    QString* error)
{
    if (artifact.files.isEmpty() || !QFileInfo(root).isDir()) {
        if (error) *error = QStringLiteral("diagnostics_v2_artifact_incomplete");
        return false;
    }
    for (const ArtifactFileSnapshot& expected : artifact.files) {
        ...
        if (relative.isEmpty() || relative == QStringLiteral("..")
            ...
            if (error && error->isEmpty()) *error = QStringLiteral("diagnostics_v2_artifact_tampered");
            return false;
        }
    }
    return true;
...
bool commitFiles(ArtifactStoreV2* store, StorageV2* storage, const TaskId& taskId,
    ...
    ArtifactId* result, QString* error, const aitrain::CancellationCallback& cancellation)
{
    ...
    for (const auto& file : files) {
        if (canceled(cancellation)) {
            if (error) *error = QStringLiteral("diagnostics_v2_canceled");
            abort();
            return false;
        }
...
QJsonObject taskStateCounts(const TaskStateCountsV2& counts)
{
...
TaskState terminalState(const WorkflowRunExecutionResultV2& result)
{
...
bool ProjectWorkspaceV2::runDiagnosticsWorkflow(const TaskId& taskId,
    const DiagnosticsWorkflowRequestV2& request, DiagnosticsWorkflowResultV2* result,
    QString* error, const aitrain::CancellationCallback& cancellation)
{
    if (!isOpen() || !taskId.isValid() || !result) {
        if (error) *error = QStringLiteral("运行 Diagnostics Bundle V2 需要已打开工作区、任务和输出对象。");
        return false;
    }
    *result = {};
    TaskSnapshot task;
    if (!storage_.task(taskId, &task, error) || task.state != TaskState::Running) {
        if (error && error->isEmpty()) *error = QStringLiteral("Diagnostics Bundle V2 必须依附运行中任务。");
        return false;
    }
    ...
    workflow.templateId = QStringLiteral("diagnostics_v2");
    ...
    for (int index = 0; index < kinds.size(); ++index) {
        ...
        step.backend = QStringLiteral("builtin_diagnostics_v2");
    ...
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
                              const aitrain::CancellationCallback& stepCancellation) -> WorkflowStepExecutionResultV2 {
        if (canceled(stepCancellation)) {
            return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                QStringLiteral("diagnostics_v2_canceled_before_probe"))};
        }
        ArtifactId output;
        QString executionError;
        if (step.kind == QStringLiteral("CollectDiagnostics")) {
            ProjectSummarySnapshotV2 projectSummary;
            ...
            for (const TaskSnapshot& item : recentTasks) {
                if (canceled(stepCancellation)) {
                    return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                        QStringLiteral("diagnostics_v2_canceled_between_storage_reads"))};
                }
                QJsonArray artifacts;
                const QVector<ArtifactSnapshotV2> storedArtifacts = storage_.artifactsForTask(item.id, &executionError);
                if (!executionError.isEmpty()) {
                    return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::InternalError, executionError)};
                }
                for (int index = 0; index < storedArtifacts.size() && index < artifactLimit; ++index) {
                    const ArtifactSnapshotV2& artifact = storedArtifacts.at(index);
                    qint64 bytes = 0;
            ...
            if (canceled(stepCancellation)) {
                return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                    QStringLiteral("diagnostics_v2_canceled_before_python_probe"))};
            }
            probes.append(runProbe(QStringLiteral("python"), {QStringLiteral("--version")},
                probeTimeoutMs, probeOutputBytes));
            if (canceled(stepCancellation)) {
                return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                    QStringLiteral("diagnostics_v2_canceled_after_python_probe"))};
            }
            ...
            if (canceled(stepCancellation)) {
                return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                    QStringLiteral("diagnostics_v2_canceled_after_nvidia_probe"))};
            }

            facts = {{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("diagnostic_facts_v2")},
                ...
                {QStringLiteral("runtimeCapabilityMatrix"), RuntimeCapabilityMatrixV2().toJson()},
                ...
                {QStringLiteral("limitations"), QJsonArray{
                    ...
                    QStringLiteral("诊断事实只陈述 V2 Storage/Query、内置能力注册表、运行时矩阵和有限本机探测结果。")}}};
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("diagnostic_facts_v2"),
                    {{QStringLiteral("diagnostic_facts_v2.json"), QJsonDocument(facts).toJson(QJsonDocument::Indented)}},
            ...
            ArtifactSnapshotV2 storedFacts;
            if (!storage_.artifact(step.inputArtifactId, &storedFacts, &executionError)
                || storedFacts.kind != QStringLiteral("diagnostic_facts_v2")) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncompatible,
                    executionError.isEmpty() ? QStringLiteral("diagnostics_v2_facts_identity_mismatch") : executionError)};
            }
            ...
            QFile file(QDir(factsRoot).filePath(QStringLiteral("diagnostic_facts_v2.json")));
            QJsonParseError parseError;
            if (!file.open(QIODevice::ReadOnly)) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncomplete,
                    QStringLiteral("diagnostics_v2_facts_file_unreadable"))};
            }
            const QJsonDocument document = QJsonDocument::fromJson(file.readAll(), &parseError);
            if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncompatible,
                    QStringLiteral("diagnostics_v2_facts_json_invalid"))};
            }
            ...
            const QJsonObject report{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("diagnostic_bundle_v2")},
            ...
            const QJsonObject manifest{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("diagnostic_bundle_v2_manifest")},
                {QStringLiteral("factsArtifactId"), storedFacts.id.toString()},
                {QStringLiteral("files"), QJsonArray{QStringLiteral("diagnostic_bundle_v2.json"),
                    QStringLiteral("diagnostic_summary_v2.md"), QStringLiteral("diagnostic_manifest_v2.json")}}};
            const QString markdown = QStringLiteral("# Diagnostics Bundle V2\n\n"
                "- 任务事实数：%1\n- 本机探测数：%2\n- 不可用/超时探测数：%3\n- 运行时依赖检查数：%4\n\n"
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("diagnostic_bundle_v2"),
                    {{QStringLiteral("diagnostic_bundle_v2.json"), QJsonDocument(report).toJson(QJsonDocument::Indented)},
                        {QStringLiteral("diagnostic_summary_v2.md"), markdown.toUtf8()},
                        {QStringLiteral("diagnostic_manifest_v2.json"), QJsonDocument(manifest).toJson(QJsonDocument::Indented)}},
                    &output, &executionError, stepCancellation)) {
                const FailureCode code = canceled(stepCancellation) ? FailureCode::Canceled : FailureCode::ArtifactIncomplete;
                return {code == FailureCode::Canceled ? WorkflowStepState::Canceled : WorkflowStepState::Failed,
                    {}, diagnosticsFailure(code, executionError)};
            ...
            return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::InternalError,
                QStringLiteral("diagnostics_v2_unknown_step"))};
        }
    ...
    if (finalState != TaskState::Succeeded && !finalFailure.isFailure()) {
        finalFailure = diagnosticsFailure(finalState == TaskState::Canceled
            ? FailureCode::Canceled : FailureCode::InternalError, QStringLiteral("diagnostics_v2_missing_terminal_failure"));
    }
    ...
    evidence.limitations.append(QStringLiteral("Diagnostics V2 的外部同步探测在单次 waitForFinished 期间不可中断；取消只在探测前后检查。"));
    evidence.limitations.append(QStringLiteral("Diagnostics V2 对探测输出实施固定上限，超出内容不会进入 Artifact。"));
...
bool ProjectWorkspaceV2::runEnvironmentCheckWorkflow(const TaskId& taskId,
...
{
    if (!isOpen() || !taskId.isValid() || !result) {
        if (error) *error = QStringLiteral("运行 Environment Check V2 需要已打开工作区、任务和输出对象。");
        return false;
    }
    *result = {};
    TaskSnapshot task;
    if (!storage_.task(taskId, &task, error) || task.state != TaskState::Running) {
        if (error && error->isEmpty()) *error = QStringLiteral("Environment Check V2 必须依附运行中任务。");
        return false;
    }
    ...
    workflow.id = WorkflowRunId::create();
    workflow.taskId = taskId;
    workflow.templateId = QStringLiteral("environment_check_v2");
    workflow.terminalPolicy = WorkflowTerminalPolicyV2::EvidenceRequired;
    workflow.createdAt = QDateTime::currentDateTimeUtc();
    QVector<WorkflowStepSnapshotV2> steps;
    ...
    for (int index = 0; index < kinds.size(); ++index) {
        ...
        step.ordinal = index;
        step.kind = kinds.at(index);
        step.backend = QStringLiteral("builtin_environment_check_v2");
    ...
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
        ...
        if (canceled(stepCancellation)) {
            return {WorkflowStepState::Canceled, {}, diagnosticsFailure(FailureCode::Canceled,
                QStringLiteral("environment_check_v2_canceled"))};
        }
        ArtifactId output;
        QString executionError;
        if (step.kind == QStringLiteral("ValidateEnvironmentFacts")) {
            ...
            if (!validEnvironmentFacts(facts)
                ...
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::InvalidRequest,
                    QStringLiteral("environment_check_v2_facts_invalid"))};
            }
            QJsonObject storedFacts = facts;
            storedFacts.insert(QStringLiteral("schemaVersion"), 2);
            storedFacts.insert(QStringLiteral("kind"), QStringLiteral("environment_facts_v2"));
            if (!commitFiles(artifactStore_.get(), &storage_, taskId,
                    QStringLiteral("environment_facts_v2"),
                    {{QStringLiteral("environment_facts_v2.json"),
            ...
            ArtifactSnapshotV2 factsArtifact;
            if (!storage_.artifact(step.inputArtifactId, &factsArtifact, &executionError)
                || factsArtifact.kind != QStringLiteral("environment_facts_v2")) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncompatible,
                    QStringLiteral("environment_check_v2_facts_identity_mismatch"))};
            }
            ...
            QFile file(QDir(root).filePath(QStringLiteral("environment_facts_v2.json")));
            QJsonParseError parseError;
            if (!file.open(QIODevice::ReadOnly)) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncomplete,
                    QStringLiteral("environment_check_v2_facts_unreadable"))};
            }
            const QJsonDocument document = QJsonDocument::fromJson(file.readAll(), &parseError);
            if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
                return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::ArtifactIncompatible,
                    QStringLiteral("environment_check_v2_facts_json_invalid"))};
            }
            ...
            report.insert(QStringLiteral("kind"), QStringLiteral("environment_profiles_report_v2"));
            report.insert(QStringLiteral("factsArtifactId"), factsArtifact.id.toString());
            report.insert(QStringLiteral("summary"), summary);
            const QJsonObject manifest{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("environment_profiles_report_v2_manifest")},
                {QStringLiteral("factsArtifactId"), factsArtifact.id.toString()},
                {QStringLiteral("files"), QJsonArray{QStringLiteral("environment_profiles_report_v2.json"),
                    QStringLiteral("environment_profiles_manifest_v2.json")}}};
            if (!commitFiles(artifactStore_.get(), &storage_, taskId,
                    QStringLiteral("environment_profiles_report_v2"),
                    {{QStringLiteral("environment_profiles_report_v2.json"),
            ...
            return {WorkflowStepState::Failed, {}, diagnosticsFailure(FailureCode::InternalError,
                QStringLiteral("environment_check_v2_unknown_step"))};
        }
        outputs.insert(step.kind, output);
        return {WorkflowStepState::Succeeded, output, {}};
    };
    ...
    if (finalState != TaskState::Succeeded && !finalFailure.isFailure()) {
        finalFailure = diagnosticsFailure(finalState == TaskState::Canceled
            ...
            QStringLiteral("environment_check_v2_missing_terminal_failure"));
    }
    if (finalState == TaskState::Canceled) {
        TaskSnapshot current;
        if (!storage_.task(taskId, &current, error)) return false;
...
bool ProjectWorkspaceV2::environmentCheckReportForTask(
    const TaskId& taskId, QJsonObject* report, QString* error) const
{
    if (!isOpen() || !taskId.isValid() || !report) {
        if (error) *error = QStringLiteral("读取 Environment Check V2 报告需要已打开工作区和有效任务。");
        return false;
    }
    const QVector<ArtifactSnapshotV2> artifacts = storage_.artifactsForTask(taskId, error);
    if (error && !error->isEmpty()) return false;
    for (auto it = artifacts.crbegin(); it != artifacts.crend(); ++it) {
        if (it->kind != QStringLiteral("environment_profiles_report_v2")) continue;
        const QString root = QDir(artifactStore_->rootPath()).filePath(
            QStringLiteral("artifacts/%1").arg(it->id.toString()));
        if (!verifyArtifact(*it, root, error)) return false;
        QFile file(QDir(root).filePath(QStringLiteral("environment_profiles_report_v2.json")));
        QJsonParseError parseError;
        if (!file.open(QIODevice::ReadOnly)) {
            if (error) *error = QStringLiteral("environment_check_v2_report_unreadable");
            return false;
        }
        const QJsonDocument document = QJsonDocument::fromJson(file.readAll(), &parseError);
        if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
            if (error) *error = QStringLiteral("environment_check_v2_report_invalid");
            return false;
        }
        ...
        *report = QJsonObject{{QStringLiteral("schemaVersion"), 2},
            {QStringLiteral("kind"), QStringLiteral("environment_profiles_report_v2")},
    ...
    if (error) *error = QStringLiteral("所选任务没有已提交的 Environment Check V2 报告。");
    return false;
}
