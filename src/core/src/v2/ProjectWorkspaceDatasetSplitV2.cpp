...
Failure splitFailure(const QString& message)
{
    ...
    failure.message = message.isEmpty() ? QStringLiteral("dataset_split_v2_failed") : message;
...
bool verifyArtifactInventory(const ArtifactSnapshotV2& artifact,
    const QString& root,
    QString* error)
{
...
bool copyPureSplitTree(const ArtifactSnapshotV2& stored,
    ...
    QString* error)
{
    ...
    for (const ArtifactFileSnapshot& expected : stored.files) {
        ...
        const QString relative = QDir::cleanPath(expected.relativePath);
        if (relative == QStringLiteral("split_plan_v2.json")
...
bool ProjectWorkspaceV2::runDatasetSplitWorkflow(const TaskId& taskId,
    ...
    const aitrain::CancellationCallback& cancellation)
{
    if (!isOpen() || !taskId.isValid() || !result
        ...
        if (error) *error = QStringLiteral("dataset_split_v2_invalid_request");
        return false;
    }
    TaskSnapshot task;
    ...
    DatasetSnapshotRecordV2 sourceSnapshot;
    ArtifactSnapshotV2 sourceArtifact;
    const DatasetDriverV2* driver = nullptr;

    WorkflowRunSnapshotV2 workflow;
    workflow.id = WorkflowRunId::create();
    workflow.taskId = taskId;
    workflow.templateId = QStringLiteral("dataset_split_v2");
    ...
    for (int ordinal = 0; ordinal < kinds.size(); ++ordinal) {
        ...
        step.backend = ordinal == 0 ? QStringLiteral("dataset_snapshot_identity_v2")
            : ordinal == 1 ? QStringLiteral("dataset_split_materializer_v2")
    ...
    DatasetSnapshotArtifactBundleV2 registeredSnapshot;
    ...
    const auto resolveSourceIdentity = [&]() {
        if (!storage_.datasetSnapshot(request.sourceSnapshotId, &sourceSnapshot, &executionError)
            ...
            || sourceArtifact.kind != QStringLiteral("dataset_snapshot_v2")) {
    ...
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
        ...
        if (step.kind == QStringLiteral("PlanSplit")) {
            if (!resolveSourceIdentity()) {
                return WorkflowStepExecutionResultV2{WorkflowStepState::Failed, {}, splitFailure(executionError)};
            }
            ...
            QString staging;
            if (!artifactStore_->begin(taskId, QStringLiteral("dataset_split_plan_v2"),
                    &planArtifactId, &staging, &executionError)
                || !writeJson(QDir(staging).filePath(QStringLiteral("dataset_split_plan_v2.json")),
            ...
            if (!artifactStore_->commit(planArtifactId, taskId, QStringLiteral("dataset_split_plan_v2"),
            ...
            return WorkflowStepExecutionResultV2{WorkflowStepState::Succeeded, planArtifactId, {}};
        }
        if (step.kind == QStringLiteral("MaterializeSplit")) {
            ...
            ArtifactSnapshotV2 planStored;
            if (!storage_.artifact(planArtifactId, &planStored, &executionError)
                || planStored.kind != QStringLiteral("dataset_split_plan_v2")
                || planStored.files.size() != 1
                || planStored.files.first().relativePath != QStringLiteral("dataset_split_plan_v2.json")) {
                return WorkflowStepExecutionResultV2{WorkflowStepState::Failed, {}, splitFailure(
            ...
            const QString planPath = QDir(planRoot).filePath(QStringLiteral("dataset_split_plan_v2.json"));
            ...
            QString staging;
            if (!artifactStore_->begin(taskId, QStringLiteral("dataset_split_v2"),
            ...
            DatasetValidationResult splitValidation;
            ...
            QString ignoredPath;
            bool commitCanceled = false;
            if (!artifactStore_->commit(splitArtifactId, taskId, QStringLiteral("dataset_split_v2"),
                    staging, &storage_, &ignoredPath, &executionError, stepCancellation, &commitCanceled)) {
            ...
            return WorkflowStepExecutionResultV2{WorkflowStepState::Succeeded, splitArtifactId, {}};
        }
        if (step.kind == QStringLiteral("RegisterSnapshot")) {
            ...
            ArtifactSnapshotV2 splitStored;
            ...
            if (!storage_.artifact(splitArtifactId, &splitStored, &executionError)
                || splitStored.kind != QStringLiteral("dataset_split_v2")
            ...
            return WorkflowStepExecutionResultV2{WorkflowStepState::Succeeded, snapshotArtifactId, {}};
        }
