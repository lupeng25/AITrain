...
QString terminalPolicyText(WorkflowTerminalPolicyV2 policy)
{
    return policy == WorkflowTerminalPolicyV2::EvidenceRequired
        ? QStringLiteral("evidence_required") : QStringLiteral("immediate");
}

bool parseTerminalPolicy(const QString& value, WorkflowTerminalPolicyV2* result)
{
    if (value == QStringLiteral("immediate")) {
        if (result) *result = WorkflowTerminalPolicyV2::Immediate;
        return true;
    }
    if (value == QStringLiteral("evidence_required")) {
        if (result) *result = WorkflowTerminalPolicyV2::EvidenceRequired;
        return true;
    }
    return false;
}

QString terminalizationStateText(WorkflowTerminalizationStateV2 state)
{
    switch (state) {
    case WorkflowTerminalizationStateV2::Sealed: return QStringLiteral("sealed");
    case WorkflowTerminalizationStateV2::EvidenceAttached: return QStringLiteral("evidence_attached");
    case WorkflowTerminalizationStateV2::Closed: return QStringLiteral("closed");
    }
    return {};
}

bool parseTerminalizationState(const QString& value, WorkflowTerminalizationStateV2* result)
{
    if (value == QStringLiteral("sealed")) {
        if (result) *result = WorkflowTerminalizationStateV2::Sealed;
        return true;
    }
    if (value == QStringLiteral("evidence_attached")) {
        if (result) *result = WorkflowTerminalizationStateV2::EvidenceAttached;
        return true;
    }
    if (value == QStringLiteral("closed")) {
        if (result) *result = WorkflowTerminalizationStateV2::Closed;
        return true;
    }
    return false;
}
...
bool scalarCount(QSqlDatabase database, const QString& statement, qint64* result, QString* error)
{
    QSqlQuery query(database);
    if (!query.exec(statement) || !query.next()) {
        if (error) *error = query.lastError().isValid()
            ? sqlError(query) : QStringLiteral("项目汇总计数查询没有返回结果。");
        return false;
    }
    bool ok = false;
...
{
    QSqlQuery query(database);
    if (!query.exec(QStringLiteral("select version from v2_schema limit 1")) || !query.next()) {
        if (error) {
            *error = query.lastError().isValid() ? sqlError(query) : QStringLiteral("V2 数据库缺少 schema 版本记录。");
        }
        return false;
    }
    if (query.value(0).toInt() != kStorageSchemaVersion) {
        if (error) {
            *error = QStringLiteral("V2 数据库 schema 版本不受支持；项目尚未上线，请删除后重新创建项目。");
        }
        return false;
    }
    return true;
...
bool parseWorkflowRun(QSqlQuery& query, WorkflowRunSnapshotV2* result, QString* error)
{
    WorkflowRunSnapshotV2 parsed;
    ...
    if (!parseTerminalPolicy(query.value(3).toString(), &parsed.terminalPolicy)) {
        if (error) *error = QStringLiteral("工作流终态策略无效。");
        return false;
    }
    parsed.createdAt = parseUtc(query.value(4).toString());
    if (parsed.templateId.isEmpty() || !parsed.createdAt.isValid()) {
        if (error) *error = QStringLiteral("工作流运行记录无效。");
        return false;
    }
...
bool parseTerminalization(QSqlQuery& query, WorkflowTerminalizationSnapshotV2* result, QString* error)
{
    WorkflowTerminalizationSnapshotV2 parsed;
...
bool parseWorkflowStep(QSqlQuery& query, WorkflowStepSnapshotV2* result, QString* error)
{
    WorkflowStepSnapshotV2 parsed;
    if (!WorkflowStepId::parse(query.value(0).toString(), &parsed.id, error)
        || !WorkflowRunId::parse(query.value(1).toString(), &parsed.workflowRunId, error)
        || !workflowStepStateFromString(query.value(4).toString(), &parsed.state)) {
        if (error && error->isEmpty()) *error = QStringLiteral("工作流步骤记录包含无效状态或标识。");
        return false;
    }
    parsed.ordinal = query.value(2).toInt();
    parsed.kind = query.value(3).toString();
    if (!query.value(5).toString().isEmpty()
        && !ArtifactId::parse(query.value(5).toString(), &parsed.inputArtifactId, error)) return false;
    if (!query.value(6).toString().isEmpty()
        && !ArtifactId::parse(query.value(6).toString(), &parsed.outputArtifactId, error)) return false;
    parsed.backend = query.value(7).toString();
    QJsonParseError jsonError;
    const QJsonDocument parameters = QJsonDocument::fromJson(query.value(8).toString().toUtf8(), &jsonError);
    if (jsonError.error != QJsonParseError::NoError || !parameters.isObject()) {
        if (error) *error = QStringLiteral("工作流步骤参数摘要不是对象 JSON。");
        return false;
    }
    ...
    if (parsed.ordinal < 0 || parsed.kind.isEmpty() || parsed.backend.isEmpty() || parsed.retryCount < 0) {
...
bool parseDatasetSnapshot(QSqlQuery& query, DatasetSnapshotRecordV2* result, QString* error)
{
    DatasetSnapshotRecordV2 parsed;
    if (!DatasetId::parse(query.value(0).toString(), &parsed.datasetId, error)
        || !DatasetVersionId::parse(query.value(1).toString(), &parsed.datasetVersionId, error)
...
StorageV2::StorageV2()
    : connectionName_(QStringLiteral("aitrain_v2_%1").arg(QUuid::createUuid().toString(QUuid::Id128)))
{
}
...
bool StorageV2::open(const QString& databasePath, QString* error)
{
    close();
...
{
    QString lookupError;
    const bool hasV2Schema = hasTable(db_, QStringLiteral("v2_schema"), &lookupError);
    ...
    if (!hasV2Schema && hasTable(db_, QStringLiteral("tasks"), &lookupError)) {
        if (error) {
            *error = QStringLiteral("检测到 V1 项目数据库；V2 不支持迁移，请删除后重新创建项目。");
        }
        return false;
    }
    if (!lookupError.isEmpty()) {
    ...
    if (hasV2Schema && !hasExpectedSchemaVersion(db_, error)) {
        return false;
    }
    ...
    const QStringList statements = {
        QStringLiteral("create table if not exists v2_schema (version integer not null check(version = %1))")
            .arg(kStorageSchemaVersion),
        QStringLiteral("insert into v2_schema(version) select %1 where not exists(select 1 from v2_schema)")
            .arg(kStorageSchemaVersion),
...
bool StorageV2::appendStateEvent(const TaskId& taskId,
...
bool StorageV2::createTask(const TaskSnapshot& task, QString* error)
{
...
bool StorageV2::transitionTask(const TaskId& taskId,
...
bool StorageV2::markInterruptedTasksFailed(QString* error)
{
    QSqlQuery query(db_);
...
bool StorageV2::recordProtocolEvent(const TaskId& taskId,
    const RequestId& requestId,
    ...
    QString* error)
{
    if (!taskId.isValid() || !requestId.isValid() || !messageId.isValid() || sequence == 0 || kind.isEmpty() || !occurredAt.isValid()) {
        if (error) {
...
bool StorageV2::lastProtocolSequence(const TaskId& taskId, quint64* result, QString* error) const
{
...
bool StorageV2::recordMetric(const TaskId& taskId, const QString& name, double value, const QDateTime& occurredAt, QString* error)
{
...
bool StorageV2::recordArtifact(const ArtifactId& artifactId, const TaskId& taskId, const QString& kind, const QDateTime& createdAt, QString* error)
{
...
bool StorageV2::recordArtifactWithFiles(const ArtifactId& artifactId,
    const TaskId& taskId,
    const QString& kind,
...
bool StorageV2::recordEvidenceArtifactWithFilesAndAttachTerminalization(
    ...
    QString* error)
{
    ...
    if (artifactQuery.next()) {
        ...
        if (artifactQuery.value(0).toString() != taskId.toString()
            || artifactQuery.value(1).toString() != QStringLiteral("evidence_bundle_v2")
    ...
    if (!existing) {
        ...
        insertArtifact.prepare(QStringLiteral("insert into artifacts(id, task_id, kind, created_at) values(:id, :task_id, 'evidence_bundle_v2', :created_at)"));
...
bool StorageV2::registerDatasetSnapshot(DatasetSnapshotRecordV2* snapshot, QString* error)
{
    ...
    if (!artifactQuery.exec() || !artifactQuery.next()
        ...
        || artifactQuery.value(1).toString() != QStringLiteral("dataset_snapshot_v2")) {
        if (error) *error = artifactQuery.lastError().isValid() ? sqlError(artifactQuery)
            : QStringLiteral("数据快照必须引用同一任务已提交的 dataset_snapshot_v2 Artifact。");
        db_.rollback();
        return false;
    }
...
bool StorageV2::datasetSnapshot(const SnapshotId& snapshotId, DatasetSnapshotRecordV2* result, QString* error) const
{
    if (!snapshotId.isValid() || !result) {
        if (error) *error = QStringLiteral("查询数据集快照需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select datasets.id, dataset_versions.id, dataset_snapshots.id, dataset_snapshots.task_id, dataset_snapshots.artifact_id, dataset_snapshots.root_path, datasets.dataset_format, dataset_snapshots.driver_id, dataset_snapshots.driver_version, dataset_versions.root_hash, dataset_snapshots.manifest_sha256, dataset_snapshots.file_count, dataset_snapshots.total_bytes, dataset_snapshots.created_at from dataset_snapshots join dataset_versions on dataset_versions.id = dataset_snapshots.dataset_version_id join datasets on datasets.id = dataset_versions.dataset_id where dataset_snapshots.id = :id"));
    query.bindValue(QStringLiteral(":id"), snapshotId.toString());
    if (!query.exec() || !query.next()) {
        if (error) *error = query.lastError().isValid() ? sqlError(query) : QStringLiteral("数据集快照不存在。");
        return false;
    }
    return parseDatasetSnapshot(query, result, error);
}

bool StorageV2::datasetSnapshotForArtifact(const ArtifactId& artifactId,
    DatasetSnapshotRecordV2* result,
    QString* error) const
{
    if (!artifactId.isValid() || !result) {
        if (error) *error = QStringLiteral("按 Artifact 查询数据集快照需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select datasets.id, dataset_versions.id, dataset_snapshots.id, dataset_snapshots.task_id, dataset_snapshots.artifact_id, dataset_snapshots.root_path, datasets.dataset_format, dataset_snapshots.driver_id, dataset_snapshots.driver_version, dataset_versions.root_hash, dataset_snapshots.manifest_sha256, dataset_snapshots.file_count, dataset_snapshots.total_bytes, dataset_snapshots.created_at from dataset_snapshots join dataset_versions on dataset_versions.id = dataset_snapshots.dataset_version_id join datasets on datasets.id = dataset_versions.dataset_id where dataset_snapshots.artifact_id = :artifact_id"));
    query.bindValue(QStringLiteral(":artifact_id"), artifactId.toString());
    if (!query.exec() || !query.next()) {
        if (error) *error = query.lastError().isValid() ? sqlError(query) : QStringLiteral("Artifact 未关联数据集快照。");
        return false;
    }
    return parseDatasetSnapshot(query, result, error);
}

QVector<DatasetCatalogItemV2> StorageV2::datasets(int limit, QString* error) const
{
    QVector<DatasetCatalogItemV2> results;
    if (limit <= 0) {
    ...
    while (query.next()) {
        DatasetCatalogItemV2 item;
        if (!DatasetId::parse(query.value(0).toString(), &item.datasetId, error)) return {};
        ...
        if (!query.value(4).isNull()) {
            if (!DatasetVersionId::parse(query.value(4).toString(), &item.latestVersionId, error)
                || !SnapshotId::parse(query.value(5).toString(), &item.latestSnapshotId, error)
                || !ArtifactId::parse(query.value(6).toString(), &item.latestArtifactId, error)) return {};
...
bool StorageV2::removeUnreferencedArtifact(const ArtifactId& artifactId, QString* error)
{
...
bool StorageV2::registerModelPackage(const ModelPackageSnapshotV2& modelPackage, QString* error)
{
    const ModelManifestV2& manifest = modelPackage.manifest;
    if (!modelPackage.sourceArtifactId.isValid() || !validateModelManifestV2(manifest, error)) {
        if (error && error->isEmpty()) {
    ...
    const QJsonObject encoded = encodeModelManifestV2(manifest, &manifestError);
    if (encoded.isEmpty()) {
...
bool StorageV2::modelPackage(const ModelPackageId& modelPackageId, ModelPackageSnapshotV2* result, QString* error) const
{
    if (!modelPackageId.isValid() || !result) {
        if (error) *error = QStringLiteral("查询模型包需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    ...
    if (!ArtifactId::parse(query.value(0).toString(), &sourceArtifactId, error)) return false;
    QJsonParseError parseError;
    const QJsonDocument document = QJsonDocument::fromJson(query.value(1).toString().toUtf8(), &parseError);
    ModelManifestV2 manifest;
    if (parseError.error != QJsonParseError::NoError || !document.isObject() || !decodeModelManifestV2(document.object(), &manifest, error)) {
        if (error && error->isEmpty()) *error = QStringLiteral("已登记模型包的 Manifest JSON 无效。");
        return false;
    }
...
QVector<ModelPackageSnapshotV2> StorageV2::modelPackages(int limit, QString* error) const
{
    QVector<ModelPackageSnapshotV2> results;
    if (limit <= 0) {
...
bool StorageV2::projectSummary(ProjectSummarySnapshotV2* result, QString* error) const
{
    if (error) error->clear();
    if (!isOpen() || !result) {
        if (error) *error = QStringLiteral("查询项目汇总需要已打开的 V2 Storage 和输出对象。");
        return false;
    }

    ProjectSummarySnapshotV2 summary;
    QSqlQuery taskCounts(db_);
    if (!taskCounts.exec(QStringLiteral("select state, count(*) from tasks group by state"))) {
        if (error) *error = sqlError(taskCounts);
        return false;
    }
    while (taskCounts.next()) {
        ...
        case TaskState::CancelRequested: summary.tasks.cancelRequested = count; break;
    ...
    if (!scalarCount(db_, QStringLiteral("select count(*) from artifacts"),
            &summary.committedArtifactCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from datasets"),
            &summary.datasetCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from dataset_versions"),
            &summary.datasetVersionCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from dataset_snapshots"),
            &summary.datasetSnapshotCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from model_packages"),
            &summary.modelPackageCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from model_packages where verified = 1"),
            &summary.verifiedModelPackageCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from workflow_runs"),
            &summary.workflowRunCount, error)
        || !scalarCount(db_, QStringLiteral("select count(*) from workflow_runs where terminal_policy = 'evidence_required'"),
            &summary.evidenceRequiredWorkflowCount, error)
        || !scalarCount(db_, QStringLiteral(
                "select count(*) from workflow_terminalizations z "
                "join artifacts a on a.id = z.evidence_artifact_id "
                "where a.kind = 'evidence_bundle_v2' and z.state in ('evidence_attached','closed')"),
...
bool StorageV2::createWorkflowRun(const WorkflowRunSnapshotV2& workflow,
...
bool StorageV2::createWorkflowRunWithInput(const WorkflowRunSnapshotV2& workflow,
    const QVector<WorkflowStepSnapshotV2>& steps,
...
bool StorageV2::createWorkflowRunInternal(const WorkflowRunSnapshotV2& workflow,
    const QVector<WorkflowStepSnapshotV2>& steps,
    const WorkflowInputBindingV2* input,
    QString* error)
{
    ...
    QSet<int> ordinals;
    for (const WorkflowStepSnapshotV2& step : steps) {
    ...
    if (input) {
        ...
        ArtifactSnapshotV2 artifactSnapshot;
        ModelPackageSnapshotV2 modelPackageSnapshot;
        ...
        const QHash<QString, QString> artifactRoles{
            {QStringLiteral("dataset_repair_manifest"), QStringLiteral("dataset_repair_manifest_v2")},
            {QStringLiteral("annotation_session"), QStringLiteral("annotation_session_v2")}};
        if (datasetInput) {
            if (!input->datasetId.isValid() || !input->datasetSnapshotId.isValid()
                ...
                || input->sourceArtifactKind != QStringLiteral("dataset_snapshot_v2")
...
bool StorageV2::workflowRun(const WorkflowRunId& workflowRunId, WorkflowRunSnapshotV2* result, QString* error) const
{
    if (!workflowRunId.isValid() || !result) {
        if (error) *error = QStringLiteral("查询工作流运行需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select id, task_id, template_id, terminal_policy, created_at from workflow_runs where id = :id"));
    query.bindValue(QStringLiteral(":id"), workflowRunId.toString());
    if (!query.exec() || !query.next()) {
        if (error) *error = query.lastError().isValid() ? sqlError(query) : QStringLiteral("工作流运行不存在。");
        return false;
    }
    return parseWorkflowRun(query, result, error);
}

bool StorageV2::workflowInput(const WorkflowRunId& workflowRunId,
    const QString& role,
    WorkflowInputBindingV2* result,
    QString* error) const
{
    if (!workflowRunId.isValid() || role.trimmed().isEmpty() || !result) {
        if (error) *error = QStringLiteral("查询 Workflow 输入需要有效运行、角色和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral(
        "select workflow_run_id, role, source_artifact_id, source_task_id, source_artifact_kind, "
        "coalesce(dataset_id, ''), coalesce(dataset_snapshot_id, ''), coalesce(dataset_version_id, ''), "
        "coalesce(model_package_id, ''), manifest_sha256, root_hash, bound_at "
        "from workflow_input_bindings where workflow_run_id = :workflow_run_id and role = :role"));
    query.bindValue(QStringLiteral(":workflow_run_id"), workflowRunId.toString());
    query.bindValue(QStringLiteral(":role"), role.trimmed());
    if (!query.exec() || !query.next()) {
        if (error) *error = query.lastError().isValid()
            ? sqlError(query) : QStringLiteral("Workflow 输入绑定不存在。");
        return false;
    }
    WorkflowInputBindingV2 parsed;
    if (!WorkflowRunId::parse(query.value(0).toString(), &parsed.workflowRunId, error)
        || !ArtifactId::parse(query.value(2).toString(), &parsed.sourceArtifactId, error)
        || !TaskId::parse(query.value(3).toString(), &parsed.sourceTaskId, error)) {
        return false;
    }
    ...
    if ((!datasetId.isEmpty() && !DatasetId::parse(datasetId, &parsed.datasetId, error))
        || (!snapshotId.isEmpty() && !SnapshotId::parse(snapshotId, &parsed.datasetSnapshotId, error))
        || (!datasetVersionId.isEmpty() && !DatasetVersionId::parse(datasetVersionId, &parsed.datasetVersionId, error))
        || (!modelPackageId.isEmpty() && !ModelPackageId::parse(modelPackageId, &parsed.modelPackageId, error))) {
        return false;
    }
    ...
    if (parsed.workflowRunId != workflowRunId || parsed.role != role.trimmed()
        || parsed.sourceArtifactKind.trimmed().isEmpty() || !parsed.boundAt.isValid()) {
...
QVector<WorkflowStepSnapshotV2> StorageV2::workflowSteps(const WorkflowRunId& workflowRunId, QString* error) const
{
    QVector<WorkflowStepSnapshotV2> results;
    if (!workflowRunId.isValid()) {
        if (error) *error = QStringLiteral("查询工作流步骤需要有效运行 ID。");
        return results;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select id, workflow_run_id, ordinal, kind, state, coalesce(input_artifact_id, ''), coalesce(output_artifact_id, ''), backend, parameter_summary_json, coalesce(started_at, ''), coalesce(finished_at, ''), failure_code, failure_details, failure_suggested_action, coalesce(failure_occurred_at, ''), retry_count from workflow_steps where workflow_run_id = :workflow_run_id order by ordinal asc"));
    query.bindValue(QStringLiteral(":workflow_run_id"), workflowRunId.toString());
    if (!query.exec()) {
        if (error) *error = sqlError(query);
        return {};
    ...
    while (query.next()) {
        ...
        if (!parseWorkflowStep(query, &step, error)) return {};
        results.append(step);
    }
    return results;
}

bool StorageV2::bindWorkflowStepInput(const WorkflowStepId& workflowStepId,
...
bool StorageV2::transitionWorkflowStep(const WorkflowStepId& workflowStepId,
...
bool StorageV2::retryWorkflowStep(const WorkflowStepId& workflowStepId, QString* error)
{
...
bool StorageV2::sealWorkflowTerminalization(const WorkflowRunId& workflowRunId,
    ...
    QString* error)
{
    ...
    TaskId taskId;
    WorkflowTerminalPolicyV2 policy;
    ...
    if (policy != WorkflowTerminalPolicyV2::EvidenceRequired) {
    ...
    if (insert.numRowsAffected() == 0) {
        WorkflowTerminalizationSnapshotV2 existing;
...
bool StorageV2::workflowTerminalization(const WorkflowRunId& workflowRunId,
    WorkflowTerminalizationSnapshotV2* result,
    QString* error) const
{
    if (!workflowRunId.isValid() || !result) {
        if (error) *error = QStringLiteral("查询工作流终态封存需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select workflow_run_id, task_id, state, terminal_state, failure_code, failure_details, failure_suggested_action, coalesce(failure_occurred_at,''), terminal_at, coalesce(evidence_artifact_id,''), evidence_attempt_count, last_evidence_failure_code, last_evidence_failure_details, last_evidence_failure_suggested_action, coalesce(last_evidence_failure_occurred_at,''), sealed_at, coalesce(evidence_attached_at,''), coalesce(closed_at,'') from workflow_terminalizations where workflow_run_id = :id"));
    query.bindValue(QStringLiteral(":id"), workflowRunId.toString());
    if (!query.exec() || !query.next()) {
...
QVector<WorkflowTerminalizationSnapshotV2> StorageV2::pendingWorkflowTerminalizations(
    int limit,
    QString* error) const
{
    QVector<WorkflowTerminalizationSnapshotV2> results;
    if (limit <= 0) {
    ...
    while (query.next()) {
        WorkflowRunId id;
        if (!WorkflowRunId::parse(query.value(0).toString(), &id, error)) return {};
        ids.append(id);
    }
    query.finish();
    for (const WorkflowRunId& id : ids) {
        WorkflowTerminalizationSnapshotV2 item;
        if (!workflowTerminalization(id, &item, error)) return {};
        results.append(item);
    }
    return results;
}

QVector<WorkflowRunSnapshotV2> StorageV2::pendingEvidenceRequiredWorkflows(
    int limit,
    QString* error) const
{
    QVector<WorkflowRunSnapshotV2> results;
    if (limit <= 0) {
    ...
    for (const WorkflowRunId& id : ids) {
        ...
        if (!workflowRun(id, &workflow, error)) return {};
        results.append(workflow);
    }
    return results;
}

bool StorageV2::attachWorkflowTerminalizationEvidence(const WorkflowRunId& workflowRunId,
    ...
    QString* error)
{
    ...
    if (!artifactQuery.exec() || !artifactQuery.next()
        ...
        || artifactQuery.value(1).toString() != QStringLiteral("evidence_bundle_v2")) {
        if (error) *error = artifactQuery.lastError().isValid() ? sqlError(artifactQuery)
            : QStringLiteral("Evidence 必须是同一根任务已提交的 evidence_bundle_v2 Artifact。");
        db_.rollback();
        return false;
    }
...
bool StorageV2::recordWorkflowTerminalizationEvidenceFailure(const WorkflowRunId& workflowRunId,
    const Failure& failure,
    QString* error)
{
...
bool StorageV2::closeWorkflowTerminalization(const WorkflowRunId& workflowRunId,
    ...
    QString* error)
{
    ...
    WorkflowTerminalizationSnapshotV2 terminalization;
    ...
    if (terminalization.state == WorkflowTerminalizationStateV2::Closed) {
        if (currentTaskState == terminalization.terminalState) return db_.commit();
        if (error) *error = QStringLiteral("已关闭终态与根任务状态不一致。");
        db_.rollback();
        return false;
    }
    if (terminalization.state != WorkflowTerminalizationStateV2::EvidenceAttached
        || query.value(20).toString() != terminalization.taskId.toString()
        || query.value(21).toString() != QStringLiteral("evidence_bundle_v2")) {
        if (error) *error = QStringLiteral("关闭工作流终态前必须关联同任务 evidence_bundle_v2 Artifact。");
        db_.rollback();
        return false;
    }
...
bool StorageV2::artifact(const ArtifactId& artifactId, ArtifactSnapshotV2* result, QString* error) const
{
    if (!artifactId.isValid() || !result) {
        if (error) *error = QStringLiteral("查询 Artifact 需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery artifactQuery(db_);
    artifactQuery.prepare(QStringLiteral("select id, task_id, kind, created_at from artifacts where id = :id"));
    artifactQuery.bindValue(QStringLiteral(":id"), artifactId.toString());
    if (!artifactQuery.exec() || !artifactQuery.next()) {
        if (error) *error = artifactQuery.lastError().isValid()
                ? sqlError(artifactQuery) : QStringLiteral("Artifact 不存在。");
        return false;
    }
    ArtifactSnapshotV2 parsed;
    ...
    QSqlQuery filesQuery(db_);
    filesQuery.prepare(QStringLiteral("select relative_path, sha256, byte_count from artifact_files where artifact_id = :artifact_id order by relative_path asc"));
    filesQuery.bindValue(QStringLiteral(":artifact_id"), artifactId.toString());
    if (!filesQuery.exec()) {
        if (error) *error = sqlError(filesQuery);
        return false;
...
QVector<ArtifactSnapshotV2> StorageV2::artifactsForTask(const TaskId& taskId, QString* error) const
{
    QVector<ArtifactSnapshotV2> results;
    if (!taskId.isValid()) {
...
QVector<MetricSnapshotV2> StorageV2::metricsForTask(const TaskId& taskId, QString* error) const
{
    QVector<MetricSnapshotV2> results;
    if (!taskId.isValid()) {
...
bool StorageV2::taskExists(const TaskId& taskId, bool* exists, QString* error) const
{
    if (!taskId.isValid() || !exists) {
        if (error) {
            *error = QStringLiteral("查询任务存在性需要有效 ID 和输出对象。");
        }
        return false;
    }
    QSqlQuery query(db_);
...
bool StorageV2::artifactExists(const ArtifactId& artifactId, bool* exists, QString* error) const
{
    if (!artifactId.isValid() || !exists) {
        if (error) *error = QStringLiteral("查询 Artifact 存在性需要有效 ID 和输出对象。");
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select 1 from artifacts where id = :id"));
...
bool StorageV2::task(const TaskId& taskId, TaskSnapshot* result, QString* error) const
{
    if (!taskId.isValid() || !result) {
        if (error) {
            *error = QStringLiteral("查询任务需要有效 ID 和输出对象。");
        }
        return false;
    }
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select id, request_id, state, capability_id, task_type, created_at, updated_at, failure_code, failure_details, failure_suggested_action, coalesce(failure_occurred_at, '') from tasks where id = :id"));
    query.bindValue(QStringLiteral(":id"), taskId.toString());
    if (!query.exec() || !query.next()) {
        if (error) {
            *error = query.lastError().isValid() ? sqlError(query) : QStringLiteral("任务不存在。");
        }
        return false;
    }
...
QVector<TaskSnapshot> StorageV2::tasks(int limit, QString* error) const
{
...
QVector<WorkflowRunSnapshotV2> StorageV2::workflowRunsForTask(const TaskId& taskId, QString* error) const
{
    QVector<WorkflowRunSnapshotV2> results;
    if (!taskId.isValid()) {
    ...
    while (query.next()) {
        WorkflowRunSnapshotV2 snapshot;
...
int StorageV2::eventCount(const TaskId& taskId, QString* error) const
{
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select count(*) from task_events where task_id = :task_id"));
    query.bindValue(QStringLiteral(":task_id"), taskId.toString());
    if (!query.exec() || !query.next()) {
        if (error) {
            *error = sqlError(query);
...
int StorageV2::metricCount(const TaskId& taskId, QString* error) const
{
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select count(*) from task_metrics where task_id = :task_id"));
    query.bindValue(QStringLiteral(":task_id"), taskId.toString());
    if (!query.exec() || !query.next()) {
        if (error) {
            *error = sqlError(query);
...
int StorageV2::artifactCount(const TaskId& taskId, QString* error) const
{
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select count(*) from artifacts where task_id = :task_id"));
    query.bindValue(QStringLiteral(":task_id"), taskId.toString());
    if (!query.exec() || !query.next()) {
        if (error) {
            *error = sqlError(query);
...
int StorageV2::artifactFileCount(const ArtifactId& artifactId, QString* error) const
{
    QSqlQuery query(db_);
    query.prepare(QStringLiteral("select count(*) from artifact_files where artifact_id = :artifact_id"));
    query.bindValue(QStringLiteral(":artifact_id"), artifactId.toString());
    if (!query.exec() || !query.next()) {
        if (error) {
            *error = sqlError(query);
