    ...
    QString* error)
{
    ...
    if (!createDatasetSnapshotV2(root, temporary.filePath(QStringLiteral("manifest.json")),
...
ValidatedDatasetDriverV2::ValidatedDatasetDriverV2(QString driverId,
    QString format,
    Validator validator,
    Splitter splitter,
...
QString ValidatedDatasetDriverV2::id() const
{
    return driverId_;
}

QString ValidatedDatasetDriverV2::version() const
{
    return QStringLiteral("2.0");
}

QStringList ValidatedDatasetDriverV2::supportedFormats() const
{
    return {format_};
}

bool ValidatedDatasetDriverV2::detect(const QString& sourcePath,
...
bool ValidatedDatasetDriverV2::inspect(const QString& sourcePath,
    const QString& format,
    DatasetInspection* inspection,
    const DatasetOperationContext& context,
    QString* error) const
...
bool ValidatedDatasetDriverV2::validate(const DatasetInspection& inspection,
...
bool ValidatedDatasetDriverV2::planSplit(const DatasetInspection& inspection,
    ...
    QString* error) const
{
    ...
    // V2 Driver 的输入边界固定为 sourceRoot；禁止旧校验器通过可选绝对路径读取根目录外的标签或字典。
...
bool ValidatedDatasetDriverV2::materializeSplit(const DatasetSplitPlan& plan,
    ...
    QString* error) const
{
    ...
    QSaveFile manifestFile(QDir(stagingRoot).filePath(QStringLiteral("split_plan_v2.json")));
...
bool ValidatedDatasetDriverV2::snapshot(const DatasetInspection& inspection,
    ...
    QString* error) const
{
    ...
    return createDatasetSnapshotV2(inspection.sourcePath, manifestPath, format_, id(), version(), options, result, error);
}
