...
FailureCode failureCodeForRuntimeStatus(RuntimeStatusV2 status)
{
    switch (status) {
    case RuntimeStatusV2::RuntimeNotImplemented: return FailureCode::RuntimeNotImplemented;
    case RuntimeStatusV2::SdkMissing: return FailureCode::SdkMissing;
    case RuntimeStatusV2::DependencyMissing: return FailureCode::DependencyMissing;
    case RuntimeStatusV2::HardwareUnsupported: return FailureCode::HardwareUnsupported;
    case RuntimeStatusV2::ArtifactIncompatible: return FailureCode::ArtifactIncompatible;
    case RuntimeStatusV2::Available: return FailureCode::None;
    }
    return FailureCode::InternalError;
}

Failure runtimeFailure(RuntimeStatusV2 status, const QString& message)
{
    QString action = QStringLiteral("检查 Model Manifest、已提交 Artifact 和目标 Runtime 能力状态后重试。");
    if (status == RuntimeStatusV2::SdkMissing) action = QStringLiteral("安装并重新配置目标 Runtime SDK 后重试。");
    if (status == RuntimeStatusV2::DependencyMissing) action = QStringLiteral("补齐目标 Runtime DLL/依赖并重新执行环境检查。");
    if (status == RuntimeStatusV2::HardwareUnsupported) action = QStringLiteral("改用满足产品要求的硬件，不能把软件缺失记为硬件不支持。");
    if (status == RuntimeStatusV2::RuntimeNotImplemented) action = QStringLiteral("选择已实现的 Runtime/decoder 组合；不要隐式降级到其他后端。");
    return {failureCodeForRuntimeStatus(status),
        message.isEmpty() ? QStringLiteral("Runtime 操作失败：%1").arg(runtimeStatusV2ToString(status)) : message,
        action, QDateTime::currentDateTimeUtc()};
}

WorkflowStepExecutionResultV2 failedExecution(const Failure& failure)
{
    return {failure.code == FailureCode::Canceled ? WorkflowStepState::Canceled : WorkflowStepState::Failed, {}, failure};
}

WorkflowStepExecutionResultV2 canceledExecution(const QString& message)
{
    return failedExecution({FailureCode::Canceled, message,
        QStringLiteral("确认取消原因后可从新的任务重新执行。"), QDateTime::currentDateTimeUtc()});
}

std::unique_ptr<RuntimeAdapterV2> runtimeAdapter(const QString& route)
{
    if (route == QStringLiteral("aitrain_onnxruntime")) return std::make_unique<OnnxRuntimeAdapterV2>();
    if (route == QStringLiteral("aitrain_ncnn")) return std::make_unique<NcnnRuntimeAdapterV2>();
    if (route == QStringLiteral("aitrain_tensorrt")) return std::make_unique<TensorRtRuntimeAdapterV2>();
    return {};
}
...
QJsonObject persistedRuntimeDetails(const RuntimeOperationResultV2& operation)
{
...
RuntimeArtifactCandidateV2 runtimeCandidate(const QString& kind, const QString& sourcePath)
{
    RuntimeArtifactCandidateV2 candidate;
    candidate.kind = kind;
...
bool ProjectWorkspaceV2::runRuntimeDeliveryWorkflow(const TaskId& taskId,
    ...
    RuntimeAdapterFactoryV2 adapterFactory)
{
    ...
    *result = {};
    ...
    ArtifactSnapshotV2 sourceArtifact;
    ...
    workflow.templateId = QStringLiteral("runtime-delivery-v2");
    ...
    for (int ordinal = 0; ordinal < kRuntimeDeliverySteps.size(); ++ordinal) {
        ...
        step.backend = ordinal == 0 ? QStringLiteral("model_package_registry_v2")
            : (ordinal == kRuntimeDeliverySteps.size() - 1 ? QStringLiteral("evidence_renderer_v2")
    ...
    ModelPackageRuntimeServiceV2 resolver(&storage_, artifactStore_->rootPath());
    RuntimeModelLocationV2 modelLocation;
    RuntimeCapabilityV2 capability;
    std::unique_ptr<RuntimeAdapterV2> adapter;
    RuntimeStatusV2 observedStatus = RuntimeStatusV2::RuntimeNotImplemented;
    ...
    const auto observe = [&](RuntimeStatusV2 status, const QString& message) {
        observedStatus = status;
        observedMessage = message;
        statusObserved = true;
    };
    const auto commitFiles = [&](const WorkflowStepSnapshotV2& step,
                                 const QString& bundleKind,
                                 const QVector<RuntimeArtifactCandidateV2>& candidates,
                                 ArtifactId* artifactId,
                                 QString* commitError) -> bool {
        RuntimeArtifactBundleV2 bundle;
        ...
        fact.insert(QStringLiteral("status"), runtimeStatusV2ToString(observedStatus));
    ...
    WorkflowRunExecutionResultV2 executionResult;
        ...
        [&](const WorkflowStepSnapshotV2& step,
            ...
            if (step.kind == QStringLiteral("ImportOrResolveModel")) {
                ...
                if (!storage_.artifactExists(modelPackage.sourceArtifactId, &exists, &stepError) || !exists) {
                    observe(RuntimeStatusV2::ArtifactIncompatible,
            ...
            if (step.kind == QStringLiteral("ValidateManifest")) {
                ...
                if (!resolver.resolve(request.modelPackageId, request.runtimeRoute, &modelLocation, &capability, &resolveError)) {
                    RuntimeStatusV2 status = capability.runtimeStatus;
                    if (status == RuntimeStatusV2::Available) status = RuntimeStatusV2::RuntimeNotImplemented;
                ...
                if (!adapter) {
                    observe(RuntimeStatusV2::RuntimeNotImplemented,
                        QStringLiteral("该 Runtime 路由没有 AITrain C++ V2 Adapter；外部官方路线不能伪装成本地 Runtime。"));
                    return failedExecution(runtimeFailure(observedStatus, observedMessage));
                }
                RuntimeOperationResultV2 operation = adapter->validateModel(modelLocation);
                if (operation.status == RuntimeStatusV2::Available) operation = adapter->probe(modelLocation);
                observe(operation.status, operation.message);
                if (operation.status != RuntimeStatusV2::Available) {
                ...
                const QJsonObject manifest = encodeModelManifestV2(modelLocation.manifest, &manifestError);
                QJsonObject report;
                ...
                report.insert(QStringLiteral("runtimeStatus"), runtimeStatusV2ToString(operation.status));
                ...
                ArtifactId output;
                QVector<RuntimeArtifactCandidateV2> candidates;
                candidates.append(runtimeCandidate(QStringLiteral("manifest_validation_report"), reportPath));
                if (!commitFiles(step, QStringLiteral("runtime_manifest_validation_v2"),
            ...
            if (step.kind == QStringLiteral("RunInferenceSmoke")
                ...
                if (!adapter) {
                    observe(RuntimeStatusV2::RuntimeNotImplemented, QStringLiteral("Runtime Adapter 尚未通过 ValidateManifest。"));
                    return failedExecution(runtimeFailure(observedStatus, observedMessage));
                }
                ...
                RuntimeOperationResultV2 operation;
                ...
                operationReport.insert(QStringLiteral("runtimeStatus"), runtimeStatusV2ToString(operation.status));
                ...
                ArtifactId output;
                const QString bundleKind = step.kind == QStringLiteral("RunInferenceSmoke") ? QStringLiteral("runtime_inference_smoke_v2")
                    : (step.kind == QStringLiteral("Benchmark") ? QStringLiteral("runtime_benchmark_v2") : QStringLiteral("runtime_deployment_validation_v2"));
                QVector<RuntimeArtifactCandidateV2> candidates;
                ...
                if (step.kind == QStringLiteral("Benchmark")) {
                    ...
                    benchmarkFacts.insert(QStringLiteral("runtimeStatus"), runtimeStatusV2ToString(operation.status));
            ...
            if (step.kind == QStringLiteral("RenderDeliveryReport")) {
                ...
                report.insert(QStringLiteral("runtimeStatus"),
                    statusObserved ? runtimeStatusV2ToString(observedStatus) : QStringLiteral("not_probed"));
                ...
                const QString markdown = QStringLiteral("# Runtime Workflow Delivery Summary V2\n\n"
                    "- Model Package：`%1`\n- Runtime：`%2`\n- Runtime Status：`%3`\n- Sample SHA-256：`%4`\n\n"
                    ...
                    .arg(request.modelPackageId.toString(), request.runtimeRoute,
                        statusObserved ? runtimeStatusV2ToString(observedStatus) : QStringLiteral("not_probed"), sampleSha256);
                ...
                if (!commitFiles(step, QStringLiteral("runtime_delivery_report_v2"),
                        candidates, &output, &stepError)) {
    ...
    evidence.evaluation.insert(QStringLiteral("reason"),
        QStringLiteral("V2-503 是推理/部署 Workflow，不重新执行训练评估。"));
    ...
    EvidenceArtifactBundleV2 evidenceArtifact;
