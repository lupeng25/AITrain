...
constexpr auto kArtifactKind = "dataset_conversion_v2";
...
QString frozenSourcePath(const DatasetConversionRequestV2& request)
{
...
bool addCommonOutputs(QJsonArray* outputs, QSet<QString>* targets, QString* error)
{
    return addTarget(outputs, targets, QStringLiteral("data.yaml"), error)
        && addTarget(outputs, targets, QStringLiteral("conversion_plan_v2.json"), error)
        && addTarget(outputs, targets, QStringLiteral("dataset_conversion_report.json"), error)
        && addTarget(outputs, targets, QStringLiteral("conversion_commit_report_v2.json"), error);
}
...
bool planCocoToYolo(const DatasetConversionRequestV2& request,
...
bool planVocToYolo(const DatasetConversionRequestV2& request,
    const QSet<QString>& frozenSourcePaths,
    QJsonArray* outputs,
    QString* error)
{
...
bool validateConversionRoute(const DatasetConversionRequestV2& request, QString* error)
{
    ...
    QString reason = QStringLiteral("no_v2_target_driver");
    if (source == QStringLiteral("xanylabeling_xlabel") || target == QStringLiteral("xanylabeling_xlabel")) {
        ...
        reason = QStringLiteral("coco_or_voc_target_has_no_v2_driver");
    }
...
bool buildPlan(const DatasetConversionRequestV2& request,
    ...
    QString* error)
{
    ...
    QJsonObject value{{QStringLiteral("schemaVersion"), 2},
        ...
        {QStringLiteral("deferredOutputs"), QJsonArray{QStringLiteral("conversion_commit_report_v2.json")}},
...
bool sourceStillMatches(const DatasetConversionRequestV2& request,
    const QJsonObject& plan,
    const aitrain::CancellationCallback& cancellation,
    QString* error)
{
...
QJsonArray validationIssues(const DatasetValidationResult& validation)
{
    return validation.issues;
}

bool failInjected(const DatasetConversionFailureInjectorV2& injector,
    DatasetConversionFailPointV2 point,
...
bool ioFailureInjected(const DatasetConversionIoFailureInjectorV2& injector,
    DatasetConversionIoOperationV2 operation,
    const QString& path,
    QString* error)
{
...
DatasetConversionServiceV2::DatasetConversionServiceV2(ArtifactStoreV2* artifactStore,
    StorageV2* storage,
...
bool DatasetConversionServiceV2::convert(const TaskId& taskId,
    ...
    const std::function<void(int, const QString&)>& progress) const
{
    if (!artifactStore_ || !storage_ || !storage_->isOpen() || !drivers_ || !result
        ...
        if (error) *error = QStringLiteral("dataset_conversion_v2_invalid_arguments");
        return false;
    }
    ...
    const DatasetDriverV2* targetDriver = drivers_->driverForFormat(request.targetFormat);
    if (!targetDriver) {
    ...
    if (!collectSourceEntries(frozenSourcePath(request), &sourceEntries, &sourceRoot, cancellation, error)
        ...
        || !writeJson(QDir(stagingPath).filePath(QStringLiteral("conversion_plan_v2.json")), plan, error)
        || failInjected(failureInjector_, DatasetConversionFailPointV2::AfterPlan, stagingPath, error)
    ...
    if (ioFailureInjected(ioFailureInjector_, DatasetConversionIoOperationV2::MaterializeWrite,
            stagingPath, error)) {
        abort();
        return false;
    }
    ...
    if (failInjected(failureInjector_, DatasetConversionFailPointV2::AfterMaterialize, stagingPath, error)
        || !sourceStillMatches(request, plan, cancellation, error)
    ...
    if (failInjected(failureInjector_, DatasetConversionFailPointV2::AfterTargetValidation, stagingPath, error)) {
        abort();
        return false;
    }
    ...
    const QString reportPath = QDir(stagingPath).filePath(QStringLiteral("conversion_commit_report_v2.json"));
    if (ioFailureInjected(ioFailureInjector_, DatasetConversionIoOperationV2::ReportWrite,
            reportPath, error)
    ...
    if (ioFailureInjected(ioFailureInjector_, DatasetConversionIoOperationV2::Commit,
    ...
    if (!artifactStore_->commit(artifactId, taskId, QString::fromLatin1(kArtifactKind), stagingPath,
        ...
        // ArtifactStoreV2 journal 在下次 recoverStaging 时恢复，不能删除半提交事实。
        if (QFileInfo::exists(stagingPath)) abort();
        return false;
    }
    result->artifactId = artifactId;
    result->artifactPath = artifactPath;
    result->planPath = QDir(artifactPath).filePath(QStringLiteral("conversion_plan_v2.json"));
    result->conversionReportPath = QDir(artifactPath).filePath(QStringLiteral("conversion_commit_report_v2.json"));
    result->plan = plan;
