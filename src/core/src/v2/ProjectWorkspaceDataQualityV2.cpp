...
bool loadVerifiedSnapshot(StorageV2* storage, ArtifactStoreV2* artifacts,
...
bool commitFiles(ArtifactStoreV2* artifacts, StorageV2* storage, const TaskId& taskId,
...
TaskState taskStateForResult(const WorkflowRunExecutionResultV2& result)
{
...
Failure terminalFailure(const WorkflowRunExecutionResultV2& result)
{
...
bool ProjectWorkspaceV2::runDataQualityWorkflow(const TaskId& taskId,
    const DataQualityWorkflowRequestV2& request, DataQualityWorkflowResultV2* result,
    QString* error, const aitrain::CancellationCallback& cancellation)
{
    ...
    DatasetSnapshotRecordV2 snapshotRecord;
    ...
    if (!storage_.datasetSnapshot(request.snapshotId, &snapshotRecord, error)
        ...
        || snapshotArtifact.kind != QStringLiteral("dataset_snapshot_v2")) return false;

    WorkflowRunSnapshotV2 workflow;
    workflow.id = WorkflowRunId::create();
    workflow.taskId = taskId;
    workflow.templateId = QStringLiteral("dataset_quality_v2");
    ...
    for (int index = 0; index < kinds.size(); ++index) {
        ...
        step.backend = QStringLiteral("builtin_dataset_quality_v2");
    ...
    if (!registerBuiltinDatasetDriversV2(&drivers, &executionError)) {
        ...
        if (step.kind == QStringLiteral("ValidateSnapshot")) {
            if ((request.datasetId.isValid() && request.datasetId != snapshotRecord.datasetId)
                ...
                return {WorkflowStepState::Failed, {}, makeFailure(FailureCode::ArtifactIncompatible,
                    ...
                    QStringLiteral("从 V2 数据集快照记录重新复制 DatasetId、DatasetVersionId、SnapshotId 和 ArtifactId。"))};
            }
            ...
            const DatasetDriverV2* driver = drivers.driverForFormat(verifiedSnapshot.record.datasetFormat);
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("dataset_snapshot_validation_v2"),
            ...
            if (!analyzeQuality(verifiedSnapshot, request.options, &analysis, stepCancellation, &executionError)) {
                ...
                return {wasCanceled ? WorkflowStepState::Canceled : WorkflowStepState::Failed, {},
                    makeFailure(wasCanceled ? FailureCode::Canceled : FailureCode::BackendUnsupported,
                        executionError, QStringLiteral("确认 Dataset Driver 格式属于 V2 七格式质量规则范围。"))};
            }
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("dataset_quality_analysis_v2"),
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("dataset_repair_manifest_v2"),
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("dataset_quality_report_v2"),
