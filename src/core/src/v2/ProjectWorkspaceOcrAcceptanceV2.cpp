...
WorkflowStepExecutionResultV2 failed(const Failure& value)
{
...
bool resolveReport(StorageV2* storage, const ArtifactStoreV2* store,
...
bool validThresholds(const OcrAcceptanceWorkflowRequestV2& request)
{
...
bool ProjectWorkspaceV2::runOcrAcceptanceWorkflow(const TaskId& taskId,
    ...
    *result = {};
    ...
    const QVector<ReportContract> contracts{
        {QStringLiteral("det"), request.detReportArtifactId,
            QStringLiteral("paddleocr_det_official_report_v2"),
        ...
        {QStringLiteral("rec"), request.recReportArtifactId,
            QStringLiteral("paddleocr_rec_official_report_v2"),
        ...
        {QStringLiteral("system"), request.systemReportArtifactId,
            QStringLiteral("paddleocr_system_official_report_v2"),
    ...
    workflow.templateId = QStringLiteral("ocr-acceptance-v2");
    ...
    for (int ordinal = 0; ordinal < kSteps.size(); ++ordinal) {
        ...
        step.backend = ordinal == 3 ? QStringLiteral("evidence_renderer_v2")
                                    : QStringLiteral("paddleocr_official_acceptance_v2");
    ...
    WorkflowRunExecutionResultV2 execution;
        ...
        [&](const WorkflowStepSnapshotV2& step,
            ...
            if (step.kind == QStringLiteral("ResolveEvidence")) {
                ...
                if (!commitReport(QStringLiteral("01-resolve-evidence"), QStringLiteral("resolved_evidence.json"),
                        QStringLiteral("ocr_acceptance_resolved_evidence_v2"), document, &output, &stepError)) {
            ...
            if (step.kind == QStringLiteral("ValidateOfficialReports")) {
                ...
                if (!commitReport(QStringLiteral("02-validate-official-reports"),
                        ...
                        QStringLiteral("ocr_acceptance_official_report_validation_v2"), document, &output, &stepError)) {
                    return failed(failure(FailureCode::ArtifactIncomplete,
                        QStringLiteral("ocr_acceptance.output_commit_failed"), stepError));
                }
            ...
            if (step.kind == QStringLiteral("EvaluateThresholds")) {
                ...
                if (!commitReport(QStringLiteral("03-evaluate-thresholds"),
                        ...
                        QStringLiteral("ocr_acceptance_threshold_evaluation_v2"), document, &output, &stepError)) {
                    return failed(failure(FailureCode::ArtifactIncomplete,
                        QStringLiteral("ocr_acceptance.output_commit_failed"), stepError));
                }
            ...
            if (step.kind == QStringLiteral("RenderAcceptanceReport")) {
                ...
                acceptanceFacts.insert(QStringLiteral("kind"), QStringLiteral("ocr_acceptance_report_v2"));
                ...
                const QString markdown = QStringLiteral("# OCR 客户域验收报告 V2\n\n"
                ...
                RuntimeArtifactBundleV2 bundle;
                if (!commitRuntimeArtifacts(taskId, QStringLiteral("ocr_acceptance_report_v2"),
    ...
    evidence.evaluation = {{QStringLiteral("kind"), QStringLiteral("ocr_acceptance_v2")},
    ...
    evidence.limitations.append(QStringLiteral("OCR Acceptance V2 只消费已提交的 PaddleOCR Det/Rec/System 官方报告 ArtifactId，不接受裸报告路径。"));
