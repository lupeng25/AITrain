...
bool snapshotSource(const DatasetDriverV2* driver,
...
bool ProjectWorkspaceV2::runDatasetSnapshotImportWorkflow(const TaskId& taskId,
    ...
    const aitrain::CancellationCallback& cancellation)
{
    ...
    const DatasetDriverV2* driver = drivers.driverForFormat(request.sourceFormat);
    ...
    workflow.templateId = QStringLiteral("dataset_snapshot_import_v2");
    ...
    for (int ordinal = 0; ordinal < 2; ++ordinal) {
        ...
        step.backend = ordinal == 0 ? QStringLiteral("dataset_driver_v2")
                                    : QStringLiteral("artifact_store_v2");
    ...
    DatasetSnapshotArtifactBundleV2 snapshotBundle;
    ...
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
        ...
        if (step.kind == QStringLiteral("PlanSnapshotImport")) {
            ...
            importPlan = QJsonObject{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("dataset_snapshot_import_plan_v2")},
            ...
            QString staging;
            if (!artifactStore_->begin(taskId, QStringLiteral("dataset_snapshot_import_plan_v2"),
                ...
                return WorkflowStepExecutionResultV2{WorkflowStepState::Failed, {}, importFailure(executionError)};
            }
            QString planPath;
            bool commitCanceled = false;
            if (!artifactStore_->commit(planArtifactId, taskId,
                    QStringLiteral("dataset_snapshot_import_plan_v2"), staging, &storage_,
        ...
        if (step.kind == QStringLiteral("MaterializeAndRegisterSnapshot")) {
            ArtifactSnapshotV2 storedPlan;
            if (!storage_.artifact(planArtifactId, &storedPlan, &executionError)
                || storedPlan.kind != QStringLiteral("dataset_snapshot_import_plan_v2")) {
                return WorkflowStepExecutionResultV2{WorkflowStepState::Failed, {},
            ...
            DatasetValidationResult stagedValidation;
            ...
            if (!copyFrozenInventory(request.sourcePath, importPlan, staging,
                ...
                || !createDatasetSnapshotV2(staging, manifestPath, request.sourceFormat,
