...
ModelImportServiceV2::ModelImportServiceV2(TaskCoordinator* coordinator, ArtifactStoreV2* artifactStore)
    : coordinator_(coordinator)
    , artifactStore_(artifactStore)
{
}

bool ModelImportServiceV2::importModel(const ModelImportRequestV2& request,
    ...
    const aitrain::CancellationCallback& cancellation)
{
    ...
    ModelManifestV2 manifest = request.manifest;
    ...
    *result = {};
    ...
    if (!validateModelManifestV2(manifest, error)) return fail(error ? *error : QStringLiteral("导入 Manifest 无效。"));
    ...
    ModelPackageSnapshotV2 modelPackage{manifest, artifactId, QDateTime::currentDateTimeUtc()};
