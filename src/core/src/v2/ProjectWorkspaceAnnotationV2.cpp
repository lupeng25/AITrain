...
QString statusText(AnnotationSyncStatusV2 status)
{
    switch (status) {
    case AnnotationSyncStatusV2::Inspected: return QStringLiteral("Inspected");
    case AnnotationSyncStatusV2::ChangesDetected: return QStringLiteral("ChangesDetected");
    case AnnotationSyncStatusV2::NoChanges: return QStringLiteral("NoChanges");
    case AnnotationSyncStatusV2::InvalidSession: return QStringLiteral("InvalidSession");
    case AnnotationSyncStatusV2::Conflict: return QStringLiteral("Conflict");
    case AnnotationSyncStatusV2::Canceled: return QStringLiteral("Canceled");
    }
...
bool readVerifiedArtifactJson(StorageV2* storage, ArtifactStoreV2* store,
    const ArtifactSnapshotV2& artifact, const QString& relativePath, QJsonObject* result,
    const aitrain::CancellationCallback& cancellation, QString* error)
{
...
bool loadBaseline(StorageV2* storage, ArtifactStoreV2* store, const SnapshotId& id,
    ...
    QString* error)
{
    ...
    if (!storage->datasetSnapshot(id, &record, error)
        ...
        || artifact.kind != QStringLiteral("dataset_snapshot_v2")
...
bool loadRepairContract(StorageV2* storage, ArtifactStoreV2* store, const ArtifactId& artifactId,
    BaselineSnapshot* baseline, QJsonObject* repair, QJsonObject* xany,
    const aitrain::CancellationCallback& cancellation, QString* error)
{
    ArtifactSnapshotV2 artifact;
    if (!storage->artifact(artifactId, &artifact, error)
        || artifact.kind != QStringLiteral("dataset_repair_manifest_v2")
...
bool loadSessionContract(StorageV2* storage, ArtifactStoreV2* store, const ArtifactId& artifactId,
    SessionContract* result, const aitrain::CancellationCallback& cancellation, QString* error)
{
    ArtifactSnapshotV2 artifact;
    QJsonObject manifest;
    if (!storage->artifact(artifactId, &artifact, error)
        || artifact.kind != QStringLiteral("annotation_session_v2")
    ...
    if (manifest.value(QStringLiteral("schemaVersion")).toInt() != 2
        || manifest.value(QStringLiteral("kind")).toString() != QStringLiteral("annotation_session_v2")
...
bool validateSessionLineage(StorageV2* storage, ArtifactStoreV2* store,
    ...
    const aitrain::CancellationCallback& cancellation, QString* error)
{
    ...
    ArtifactSnapshotV2 repairArtifact;
    QJsonObject repair;
    QJsonObject xany;
    if (!storage->artifact(session.repairArtifactId, &repairArtifact, error)
        || repairArtifact.kind != QStringLiteral("dataset_repair_manifest_v2")
...
bool validateWithDriver(const QString& root, const QString& format,
    const aitrain::CancellationCallback& cancellation, QString* error)
{
    DatasetDriverRegistryV2 registry;
    if (!registerBuiltinDatasetDriversV2(&registry, error)) return false;
    const DatasetDriverV2* driver = registry.driverForFormat(format);
...
TaskState terminalTaskState(const WorkflowRunExecutionResultV2& run)
{
...
Failure terminalFailure(const WorkflowRunExecutionResultV2& run, const QString& retryAction)
{
...
bool finishWorkflow(ProjectWorkspaceV2* workspace, StorageV2* storage, const TaskId& taskId,
    const WorkflowRunId& workflowId, const WorkflowRunExecutionResultV2& run,
    const QString& retryAction, ArtifactId* evidenceArtifactId, QString* error)
{
    ...
    EvidenceArtifactBundleV2 artifact;
...
bool ProjectWorkspaceV2::createAnnotationSession(const TaskId& taskId,
    const AnnotationSessionCreateRequestV2& request, AnnotationSessionCreateResultV2* result,
    QString* error, const aitrain::CancellationCallback& cancellation)
{
    ...
    workflow.templateId = QStringLiteral("annotation_session_create_v2");
    ...
    for (int index = 0; index < kinds.size(); ++index) {
        ...
        step.backend = QStringLiteral("builtin_annotation_session_v2");
    ...
    WorkflowRunExecutionResultV2 run;
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
        ...
        if (step.kind == QStringLiteral("ValidateRepairManifest")) {
            ...
            const QJsonObject report{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_repair_validation_v2")},
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_repair_validation_v2"),
            ...
            const QJsonObject plan{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_working_copy_plan_v2")},
                {QStringLiteral("sourceSnapshotId"), baseline.record.id.toString()},
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_working_copy_plan_v2"),
            ...
            QString staging;
            if (!artifactStore_->begin(taskId, QStringLiteral("annotation_session_v2"),
            ...
            const QJsonObject manifest{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_session_v2")},
            ...
            QString committedPath;
            if (!artifactStore_->commit(sessionArtifactId, taskId, QStringLiteral("annotation_session_v2"), staging,
    ...
    result->status = run.state == WorkflowStepState::Succeeded ? AnnotationSyncStatusV2::Inspected
        : (run.state == WorkflowStepState::Canceled ? AnnotationSyncStatusV2::Canceled : AnnotationSyncStatusV2::InvalidSession);
    return true;
}

bool ProjectWorkspaceV2::syncAnnotationSession(const TaskId& taskId,
    const AnnotationSessionSyncRequestV2& request, AnnotationSessionSyncResultV2* result,
    QString* error, const aitrain::CancellationCallback& cancellation)
{
    ...
    TaskSnapshot task;
    ArtifactSnapshotV2 inputArtifact;
    if (!storage_.task(taskId, &task, error) || task.state != TaskState::Running
    ...
    workflow.id = WorkflowRunId::create();
    workflow.taskId = taskId;
    workflow.templateId = QStringLiteral("annotation_session_sync_v2");
    workflow.terminalPolicy = WorkflowTerminalPolicyV2::EvidenceRequired;
    workflow.createdAt = QDateTime::currentDateTimeUtc();
    ...
    for (int index = 0; index < kinds.size(); ++index) {
        ...
        step.ordinal = index;
        step.kind = kinds.at(index);
        step.backend = QStringLiteral("builtin_annotation_sync_v2");
        step.parameterSummary = parameters;
    ...
    DatasetSnapshotRecordV2 newSnapshot;
    ...
    const auto executor = [&](const WorkflowStepSnapshotV2& step,
        ...
        if (step.kind == QStringLiteral("InspectSession")) {
            if (!loadSessionContract(&storage_, artifactStore_.get(), request.sessionArtifactId,
                ...
                inspection.status = AnnotationSyncStatusV2::InvalidSession;
                ...
                inspection.status = AnnotationSyncStatusV2::Conflict;
                ...
                inspection.status = AnnotationSyncStatusV2::Inspected;
            ...
            const QJsonObject report{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_session_inspection_v2")},
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_session_inspection_v2"),
                ...
                if (!resolveReadableWorkingDirectory(request.workingDirectory,
                    ...
                    if (inspection.status == AnnotationSyncStatusV2::Inspected) {
                        if (inspection.changedFiles.isEmpty()) {
                            inspection.status = AnnotationSyncStatusV2::NoChanges;
                            ...
                            inspection.status = AnnotationSyncStatusV2::ChangesDetected;
            ...
            const QJsonObject report{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_change_report_v2")},
                {QStringLiteral("status"), statusText(inspection.status)},
                {QStringLiteral("code"), inspection.code}, {QStringLiteral("message"), inspection.message},
                {QStringLiteral("changedFiles"), changed},
                {QStringLiteral("sourceSnapshotId"), inspection.session.snapshotId.toString()},
                {QStringLiteral("sourceDatasetVersionId"), inspection.session.datasetVersionId.toString()}};
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_change_report_v2"),
            ...
            if (inspection.status == AnnotationSyncStatusV2::InvalidSession
                || inspection.status == AnnotationSyncStatusV2::Conflict) {
            ...
            if (inspection.status == AnnotationSyncStatusV2::NoChanges) {
                const QJsonObject decision{{QStringLiteral("schemaVersion"), 2},
                    {QStringLiteral("kind"), QStringLiteral("annotation_sync_decision_v2")},
                    {QStringLiteral("status"), statusText(inspection.status)},
                    {QStringLiteral("newDatasetVersionCreated"), false}};
                if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_sync_decision_v2"),
                        {{QStringLiteral("sync_decision.json"), jsonBytes(decision)}}, &output,
                        &executionError, stepCancellation)) {
                    return {WorkflowStepState::Failed, {}, failure(FailureCode::ArtifactIncomplete, executionError,
                        QStringLiteral("清理 staging 后重新同步会话。"))};
                }
            } else if (inspection.status == AnnotationSyncStatusV2::ChangesDetected) {
                ...
                if (!artifactStore_->begin(taskId, QStringLiteral("dataset_snapshot_v2"),
                ...
                DatasetDriverRegistryV2 registry;
                if (!registerBuiltinDatasetDriversV2(&registry, &executionError)) {
                    abort();
                    return {WorkflowStepState::Failed, {}, failure(FailureCode::InternalError, executionError,
                        QStringLiteral("检查内置 Dataset Driver 注册。"))};
                }
                const DatasetDriverV2* driver = registry.driverForFormat(inspection.session.datasetFormat);
                ...
                if (!driver || !createDatasetSnapshotV2(staging, snapshotManifestPath,
            ...
            const QJsonObject report{{QStringLiteral("schemaVersion"), 2},
                {QStringLiteral("kind"), QStringLiteral("annotation_sync_report_v2")},
                {QStringLiteral("status"), statusText(inspection.status)},
                {QStringLiteral("code"), inspection.code}, {QStringLiteral("message"), inspection.message},
                {QStringLiteral("annotationSessionArtifactId"), request.sessionArtifactId.toString()},
                {QStringLiteral("sourceSnapshotId"), inspection.session.snapshotId.toString()},
            ...
            if (!commitFiles(artifactStore_.get(), &storage_, taskId, QStringLiteral("annotation_sync_report_v2"),
    ...
    result->workflowRunId = workflow.id;
    result->terminalState = terminalTaskState(run);
    result->status = run.state == WorkflowStepState::Canceled ? AnnotationSyncStatusV2::Canceled : inspection.status;
