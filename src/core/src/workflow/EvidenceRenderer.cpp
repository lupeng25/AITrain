...
QString artifactsMarkdown(const QVector<EvidenceArtifact>& artifacts)
{
    if (artifacts.isEmpty()) return QStringLiteral("- none\n");
    QString output;
    for (const EvidenceArtifact& artifact : artifacts) {
        output += QStringLiteral("- `%1` — %2\n").arg(artifact.artifactId.toString(), artifact.kind);
    }
    return output;
}

QString externalInputsMarkdown(const QVector<EvidenceExternalInput>& inputs)
{
    if (inputs.isEmpty()) return QStringLiteral("- none\n");
    QString output;
    for (const EvidenceExternalInput& input : inputs) {
...
QByteArray EvidenceRenderer::renderJson(const EvidenceBundle& bundle, QString* error)
{
    const QJsonObject object = encodeEvidenceBundle(bundle, error);
    return object.isEmpty() ? QByteArray{} : QJsonDocument(object).toJson(QJsonDocument::Indented);
}

QString EvidenceRenderer::renderMarkdown(const EvidenceBundle& bundle, QString* error)
{
    if (!validateEvidenceBundle(bundle, error)) return {};
    QString markdown;
    markdown += QStringLiteral("# AITrain Evidence Bundle \n\n");
...
QString EvidenceRenderer::renderHtml(const EvidenceBundle& bundle, QString* error)
{
    if (!validateEvidenceBundle(bundle, error)) return {};
    QString artifactItems;
    for (const EvidenceArtifact& artifact : bundle.artifacts) {
    ...
    QString html = QStringLiteral(
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>AITrain Evidence Bundle </title></head><body>"
        "<h1>AITrain Evidence Bundle </h1>"
...
QByteArray EvidenceRenderer::renderModelCard(const EvidenceBundle& bundle, QString* error)
{
    const QJsonObject evidence = encodeEvidenceBundle(bundle, error);
    if (evidence.isEmpty()) return {};
    // Model Card 不复制或重新计算 task/runtime 结论，只嵌入同一事实 Bundle。
    return QJsonDocument(QJsonObject{{QStringLiteral("schemaVersion"), 1},
        {QStringLiteral("kind"), QStringLiteral("aitrain_model_card")},
