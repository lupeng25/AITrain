...
bool validateEvidenceBundle(const EvidenceBundle& bundle, QString* error)
{
    if (bundle.schemaVersion != kEvidenceBundleSchemaVersion || bundle.projectIdentity.trimmed().isEmpty()
...
QJsonObject encodeEvidenceBundle(const EvidenceBundle& bundle, QString* error)
{
    ...
    return {
        ...
        {QStringLiteral("kind"), QStringLiteral("aitrain_evidence_bundle")},
...
bool decodeEvidenceBundle(const QJsonObject& object, EvidenceBundle* bundle, QString* error)
{
    if (!bundle || object.value(QStringLiteral("schemaVersion")).toInt(-1) != kEvidenceBundleSchemaVersion
        || object.value(QStringLiteral("kind")).toString() != QStringLiteral("aitrain_evidence_bundle")) {
        if (error) *error = QStringLiteral("不支持的 Evidence Bundle 。");
        return false;
    }
    EvidenceBundle parsed;
    ...
    if (!validateEvidenceBundle(parsed, error)) return false;
    *bundle = parsed;
    return true;
}
