...
bool createSnapshot(const QString& root, const QString& output, aitrain::DatasetSnapshotResult* result, QString* error, const aitrain::DatasetSnapshotOptions& options = {})
{
    return aitrain::createDatasetSnapshot(root, output, QStringLiteral("yolo_detection"),
...
bool exerciseDriver(const aitrain::DatasetDriver& driver, const QString& root, const QString& outputRoot, QString* error)
{
    aitrain::DatasetOperationContext context;
    aitrain::DatasetInspection inspection;
    if (!driver.detect(root, &inspection, context, error)) {
        return false;
    }
    aitrain::DatasetDriverValidationResult validation;
    ...
    aitrain::DatasetSplitPlan first;
    aitrain::DatasetSplitPlan second;
    ...
    if (!driver.materializeSplit(first, staging, context, error)
        || !QFileInfo::exists(QDir(staging).filePath(QStringLiteral("split_plan.json")))) {
        return false;
    }
    aitrain::DatasetSnapshotResult snapshot;
...
class DatasetSnapshotTests : public QObject {
    Q_OBJECT
...
private slots:
    ...
    void migratedDriversProvideDeterministicContract();
    void migratedDriverRejectsChangedSourceAndUnsafeStaging();
    void migratedDriverDetectsInvalidRecognizableLayout();
};

class TestDatasetDriver final : public aitrain::DatasetDriver {
public:
    ...
    bool detect(const QString&, aitrain::DatasetInspection*, const aitrain::DatasetOperationContext&, QString*) const override { return false; }
    bool inspect(const QString&, const QString&, aitrain::DatasetInspection*, const aitrain::DatasetOperationContext&, QString*) const override { return false; }
    bool validate(const aitrain::DatasetInspection&, aitrain::DatasetDriverValidationResult*, const aitrain::DatasetOperationContext&, QString*) const override { return false; }
    bool planSplit(const aitrain::DatasetInspection&, const QJsonObject&, aitrain::DatasetSplitPlan*, const aitrain::DatasetOperationContext&, QString*) const override { return false; }
    bool materializeSplit(const aitrain::DatasetSplitPlan&, const QString&, const aitrain::DatasetOperationContext&, QString*) const override { return false; }
    bool snapshot(const aitrain::DatasetInspection&, const QString&, const aitrain::DatasetSnapshotOptions&, aitrain::DatasetSnapshotResult*, QString*) const override { return false; }

private:
    QStringList formats_;
};

void DatasetSnapshotTests::createsCompleteDeterministicManifest()
{
    ...
    aitrain::DatasetSnapshotResult first;
    QString error;
    ...
    aitrain::DatasetSnapshotResult second;
    ...
    aitrain::DatasetSnapshotResult changed;
...
void DatasetSnapshotTests::rejectsFileLimitWithoutWritingManifest()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString root = directory.filePath(QStringLiteral("dataset"));
    ...
    aitrain::DatasetSnapshotOptions options;
    options.maxFileCount = 20000;
    aitrain::DatasetSnapshotResult result;
    QString error;
...
void DatasetSnapshotTests::cancellationDoesNotWriteManifest()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    const QString root = directory.filePath(QStringLiteral("dataset"));
...
void DatasetSnapshotTests::datasetDriverRegistryRejectsOverlappingFormats()
{
    aitrain::DatasetDriverRegistry registry;
    ...
    QVERIFY(registry.driverForFormat(QStringLiteral("YOLO_DETECTION")) == static_cast<const aitrain::DatasetDriver*>(&yoloDriver));
...
void DatasetSnapshotTests::semanticMaskDriverPreservesPaletteIndexes()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    ...
    aitrain::SemanticMaskDatasetDriver driver;
    aitrain::DatasetInspection inspection;
    aitrain::DatasetOperationContext context;
    QString error;
    ...
    aitrain::DatasetSplitPlan firstPlan;
    aitrain::DatasetSplitPlan secondPlan;
...
void DatasetSnapshotTests::migratedDriversProvideDeterministicContract()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    QString error;
    ...
    aitrain::YoloDetectionDatasetDriver detection;
    ...
    aitrain::YoloSegmentationDatasetDriver segmentation;
    ...
    aitrain::YoloObbDatasetDriver obb;
    ...
    aitrain::AnomalyFolderDatasetDriver anomaly;
    ...
    aitrain::PaddleOcrDetDatasetDriver det;
    ...
    aitrain::PaddleOcrRecDatasetDriver rec;
    ...
    aitrain::DatasetDriverRegistry builtins;
    QVERIFY2(aitrain::registerBuiltinDatasetDrivers(&builtins, &error), qPrintable(error));
...
void DatasetSnapshotTests::migratedDriverRejectsChangedSourceAndUnsafeStaging()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
    ...
    aitrain::YoloDetectionDatasetDriver driver;
    aitrain::DatasetOperationContext context;
    aitrain::DatasetInspection inspection;
    QString error;
    QVERIFY(driver.detect(root, &inspection, context, &error));
    aitrain::DatasetSplitPlan plan;
...
void DatasetSnapshotTests::migratedDriverDetectsInvalidRecognizableLayout()
{
    QTemporaryDir directory;
    QVERIFY(directory.isValid());
...
QTEST_MAIN(DatasetSnapshotTests)
#include "tst_dataset_snapshot.moc"
