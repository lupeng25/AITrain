    ...
    const QString& type, QString* error)
{
    ...
    return workspace->startTask(id, type, type, &task, error) ? id : aitrain::v2::TaskId{};
}
...
{
    const aitrain::v2::TaskId taskId = startTask(workspace, QStringLiteral("test.snapshot"), error);
...
Fixture createFixture(const QTemporaryDir& directory,
    aitrain::v2::ProjectWorkspaceV2* workspace, bool includeSystemAccuracy,
    QString* error)
{
...
aitrain::v2::OcrOfficialReportImportRequestV2 requestFor(const Fixture& fixture)
{
...
QString artifactRoot(const aitrain::v2::ProjectWorkspaceV2& workspace,
    const aitrain::v2::ArtifactId& id)
{
    return QDir(workspace.workspacePath()).filePath(
        QStringLiteral("artifact-store/artifacts/%1").arg(id.toString()));
}

} // namespace

class V2OcrReportPackagerTests final : public QObject {
    Q_OBJECT
...
void V2OcrReportPackagerTests::packagesThreeVerifiedReportsAndSnapshotDerivedCounts()
{
    ...
    const aitrain::v2::TaskId taskId = startTask(&workspace, QStringLiteral("ocr.report.import.v2"), &error);
...
void V2OcrReportPackagerTests::missingSystemAccuracyIsUnsupportedAndCommitsNothing()
{
    QTemporaryDir directory;
    aitrain::v2::ProjectWorkspaceV2 workspace;
    QString error;
    ...
    QCOMPARE(result.failure.code, aitrain::v2::FailureCode::BackendUnsupported);
...
void V2OcrReportPackagerTests::changedSnapshotSourceIsInvalidEvidenceAndCommitsNothing()
{
    QTemporaryDir directory;
    aitrain::v2::ProjectWorkspaceV2 workspace;
    QString error;
...
void V2OcrReportPackagerTests::cancellationCommitsNothing()
{
    QTemporaryDir directory;
    aitrain::v2::ProjectWorkspaceV2 workspace;
    QString error;
...
QTEST_MAIN(V2OcrReportPackagerTests)
#include "tst_v2_ocr_report_packager.moc"
