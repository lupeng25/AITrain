...
private slots:
    ...
    void pythonAdapterHostBoundsPostTerminalProcessLifetime();
...
void ProcessTreeTests::pythonAdapterHostBoundsPostTerminalProcessLifetime()
{
#ifdef Q_OS_WIN
    // 进程故意不退出；终态已经被下游接受后，Host 必须在有界窗口内
    // 终止它，并且只发出一次带 terminalEventSeen 的退出回调。
    aitrain::PythonAdapterLaunch launch;
    launch.program = QStringLiteral("cmd.exe");
    launch.arguments = QStringList{QStringLiteral("/c"),
        QStringLiteral("ping 127.0.0.1 -n 20 > nul")};
    launch.terminalExitTimeoutMs = 100;
    ...
    const aitrain::AdapterEventEndpoint endpoint = host.endpoint();
    QTcpSocket socket;
    socket.connectToHost(endpoint.host, endpoint.port);
    QVERIFY(socket.waitForConnected(3000));
    const QByteArray handshake = QByteArrayLiteral("{\"channel\":\"aitrain.adapter\",\"token\":\"")
    ...
    terminal.kind = QStringLiteral("event.succeeded");
    terminal.timestamp = QDateTime::currentDateTimeUtc();
    terminal.payload = QJsonObject{};
    const QByteArray encoded = aitrain::encodeProtocolMessage(terminal, &error);
    QVERIFY2(!encoded.isEmpty(), qPrintable(error));
    QCOMPARE(socket.write(encoded), encoded.size());
    QVERIFY(socket.waitForBytesWritten(3000));
    QTRY_VERIFY_WITH_TIMEOUT(socket.bytesAvailable() > 0, 3000);
    const QJsonObject ack = QJsonDocument::fromJson(socket.readLine()).object();
    QCOMPARE(ack.value(QStringLiteral("status")).toString(), QStringLiteral("accepted"));
    QCOMPARE(ack.value(QStringLiteral("terminal")).toBool(), true);
    ...
    QVERIFY(!host.isRunning());
#else
    QSKIP("Python Adapter Host lifecycle integration currently uses the Windows Job Object.");
#endif
}
