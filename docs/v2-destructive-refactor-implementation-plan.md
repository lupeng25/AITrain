...
| V2-104～105 | 已完成（任务与数据快照存储） | 新增 `aitrain_v2_storage`：新 SQLite Schema、WAL、外键、busy timeout、CAS 状态迁移、事件/指标/产物持久化，以及数据集、版本、快照到 Artifact 的可查询关系；V1 和旧 V2 开发数据库均按破坏性重构策略拒绝打开，业务表读写均由 `ProjectStore`/Workspace 收口。 | `aitrain_v2_storage_tests`、`aitrain_v2_application_tests`。 |
...
| V2-107 | 已完成（规划内核） | 不可变 `ExecutionPlanV2` 与 Capability Planner 统一校验 capability、任务、数据集、训练/评估后端、导出格式和运行时路由，并以 SHA-256 摘要供 Worker/Workflow Profile 复核；当前产品启动入口由 typed TaskCommand + Profile/Workflow 直接承载，不再维护另一套 GUI 启动 API。 | `aitrain_v2_application_tests`、`aitrain_ocr_segmentation_worker_tests`。 |
| V2-200 | 已完成（全工作流接入） | `aitrain_v2_artifact` 提供 staging、SHA-256 清单、manifest、同卷目录改名提交、`artifact_files` 原子登记与失败补偿删除，并已被 Dataset、Training、Runtime Delivery、OCR、Annotation、Diagnostics、Environment 和外部证据工作流统一使用。 | `aitrain_v2_artifact_tests`、全量 Harness。 |
...
| V2-300 | 已完成（进程树监管与 typed 控制面收口） | Windows `ProcessTreeSupervisor`/Job Object 已接入 Python Adapter Host；GUI `WorkerClient` ↔ `aitrain_worker` 本地 Socket 使用 Protocol V2，启动参数预绑定 RequestId/TaskId，只接受 `command.start_task` / `command.cancel_task`，事件使用 `event.*`，并拒绝跨请求/任务、重复 MessageId、非递增 sequence、未知 kind、超限帧和终态后事件。全部业务 handler 直接接收 `TaskCommand` 的 typed variant，不再把协议 JSON 作为业务函数参数；JSON 仅保留在协议 codec 边界。 | `aitrain_v2_process_tests`、`aitrain_ocr_segmentation_worker_tests`、`aitrain_platform_tests`。 |
| V2-302 | 已完成（独立双端通道） | Python `adapter_event_channel_v2.py` 与 C++ `AdapterEventServerV2` 已由 Worker Host 统一编排：仅接受 literal loopback 地址，采用一次性 token 握手后发送带 request/task/sequence 的 Protocol V2 JSONL 事件，限制消息大小，并拒绝无效 token、非事件、跨任务、重复或乱序消息。产品不再保留 V1 Worker 兼容入口。 | `aitrain_adapter_sdk_python_tests`、`aitrain_v2_process_tests`。 |
| V2-301 | 已完成（V2 Adapter Host、任务协调与候选产物收口） | `PythonAdapterHostV2` 与 `TaskExecutionHostV2` 先启动认证事件服务，再注入一次性 endpoint、request/task 身份和取消文件环境变量；根进程绑定 Windows Job Object，取消先写协作信号、宽限期后强制终止 Job。Host 通过 `TaskCoordinator` 持久化事件，并对无终态退出合成唯一 Failed/Canceled 终态；多步骤 Workflow 的根任务由 Workflow Runner 唯一收口，避免 Adapter 与根任务双重终态。目录候选通过 Artifact staging 提交。 | `aitrain_v2_process_tests`、`aitrain_v2_application_tests`。 |
...
| V2-400 | 已完成（Manifest 契约与 Runtime 准入底座） | 严格 `ModelManifestV2` 要求模型包、来源 task/snapshot、SHA-256、产物内相对入口、输入输出张量、预后处理/decoder、类别、opset、导出版本、runtime route 和 verified 状态；缺失/无效 Manifest 为 Unclassified/Invalid，不能进入未声明 runtime。`RuntimeAdapterV2` 统一定义 probe/load/infer/benchmark/deploymentValidate，并在调用具体适配器前校验 route、目录边界、符号链接与 SHA-256；模型导入 UI 已通过 Worker/Application Service 接入。 | `aitrain_v2_model_manifest_tests`、`aitrain_v2_application_tests`、`aitrain_delivery_acceptance_ui_tests`。 |
...
| V2-406 | 已完成（Runtime Capability Matrix） | `RuntimeCapabilityMatrixV2` 统一返回 Supported、UnsupportedByProduct、RuntimeNotImplemented、RequiresSdk、RequiresDependency、RequiresHardware、RequiresExternalEvidence，并输出对应 `runtimeStatus`；明确 OBB 不进入 NCNN、SMP 不进入 NCNN/TensorRT、异常检测与 OCR 需要各自官方/Worker 证据路径，且不再把软件、SDK、依赖、硬件和产品边界混为 `hardware-blocked`。Worker `--self-check`、EnvironmentCheckPresenter 和 DeliveryEvidencePresenter 消费同一结构化矩阵。 | `aitrain_v2_model_manifest_tests`、`aitrain_v2_runtime_adapter_tests`、`aitrain_ocr_segmentation_worker_tests`、`aitrain_delivery_acceptance_ui_tests`。 |
...
| V2-405 | 已完成（明确产品边界） | TensorRT V2 Adapter 和 `tensorrt_engine` Manifest 已完成 SDK、依赖、GPU compute capability、engine build 与 runtime inference 状态分类；官方 YOLO TensorRT decoder/真实 infer 明确返回 `RuntimeNotImplemented`，因此本轮不宣称 TensorRT 真实推理或 GPU 验收成功。真实硬件验收继续通过外部证据导入并固定为 `verified=false`，不改变产品边界。 | `aitrain_v2_runtime_adapter_tests`、外部证据 schema 测试。 |
...
| V2-505 | 已完成（统一 Renderer、终态报告 Artifact 与交付页） | `EvidenceRendererV2` 从同一个已校验 Bundle 生成 JSON、Markdown、HTML 与 Model Card；`ProjectWorkspaceV2` 仅在根任务终态后将四份报告一次性提交为 `evidence_bundle_v2`。交付页通过 `DeliveryEvidencePresenter` 查询结构化 Evidence，完整 Runtime Delivery 六步已由 Worker 唯一收口；不再保留旧交付报告页面或单步派发 API。 | `aitrain_v2_evidence_tests`、`aitrain_v2_application_tests`、`aitrain_delivery_acceptance_ui_tests`。 |
...
| V2-600～601 | 已完成（路由、查询与只读 Presenter 全部收口） | 新增只读 `WorkspaceRouter`，Sidebar 通过统一路由发出导航，UI 测试不再访问 MainWindow 私有控件；`ProjectQueryService` 只返回 Storage 中的 Task、Metric、committed Artifact、Workflow Run/Step、Dataset Catalog、Model Registry、交付证据和项目汇总。`TaskArtifactPresenter`、`ProjectSummaryPresenter`、`DatasetCatalogPresenter`、`ModelRegistryPresenter`、`EnvironmentCheckPresenter`、`DiagnosticBundlePresenter` 与 `DeliveryEvidencePresenter` 均不接收 Worker 原始消息、不写数据库，也不暴露物理路径；应用设置和授权令牌分别由 `ApplicationSettingsService`、`LicenseTokenStore` 管理。 | `aitrain_delivery_acceptance_ui_tests`、`aitrain_application_tests`、Presenter 定向测试；全量 37/37 CTest 通过。 |
...
| V2-606 | 已完成（Model Registry 与交付证据只读化） | `ProjectQueryService` 与 `ModelRegistryPresenter` 只输出 ModelPackageId、Task/Snapshot/Artifact lineage、模型合同、runtime routes、limitations、verified 与时间，不暴露 Artifact Store 路径。模型库表、推理和部署选择器统一消费该 Presenter；交付证据表统一消费 `DeliveryEvidencePresenter`，外部导入证据固定为 `verified=false`，旧裸路径模型对比、评估、流水线和导出控件已物理删除。 | `aitrain_model_registry_presenter_tests`、`aitrain_evidence_tests`、`aitrain_delivery_acceptance_ui_tests`。 |
| V2-607 | 已完成（环境、设置与 Failure Catalog 统一查询） | 环境页统一派发 `runEnvironmentCheckWorkflowV2`，Worker 在受控 staging 探测后由 Core 执行 `ValidateEnvironmentFacts → RenderEnvironmentReport` 两步 EvidenceRequired 工作流，提交环境事实、脱敏报告和 Evidence。响应只返回 Task/Workflow/Artifact ID 与摘要；GUI 由 `EnvironmentCheckPresenterV2` 按 TaskId 查询已校验 committed 报告，不消费检查 payload、不获得机器路径，也不再逐行写 legacy `environment_checks`。旧 `environmentCheck` Socket 命令/事件已删除并纳入 Unsupported 回归。`TaskArtifactPresenter` 已按 FailureCode 提供缺省建议并在详情中展示；应用设置与授权令牌已迁移到专用服务。 | 2026-07-17 Core、Worker、AITrainStudio 与三组相关测试目标构建成功；Worker、Presenter/Core 与 UI 定向 CTest 3/3 通过，最终全量 37/37 CTest 通过。 |
| V2-608 | 已完成（非阻塞控制面与 Presenter 状态收口） | GUI Worker 启动不再 `waitForStarted`；进程退出不再使用 `processEvents`/`waitForReadyRead` 排空循环；Socket 清理不再同步等待。WorkerClient 与 Worker 的外层请求/事件使用 Protocol V2 envelope 和独立双向 sequence；业务 handler 使用 typed TaskCommand，页面状态由 ApplicationEventRouter + Query/Presenter 刷新。启动失败、协议拒绝、终态排空以及窗口关闭取消→`idle`→异步 close 均唯一收口。 | `aitrain_delivery_acceptance_ui_tests`、`aitrain_ocr_segmentation_worker_tests`、`aitrain_platform_tests`、`AITrainStudio` 构建。 |
| V2-610 | 已完成（黑盒 UI 门禁） | 删除 `#define private public`，关键控件和 Router 均通过 objectName/只读状态验证；覆盖九页路由、主要工作区 Tab、统一 Runtime Delivery、环境报告表、注册身份/Artifact 边界、异步启动失败和任务取消按钮。窗口尺寸 1280×820 walkthrough 与页面切换压力通过 UI 门禁；长路径输入保持为 Qt 字符串并不进入控制面，后台压力由 Worker/Protocol 回归覆盖。 | `aitrain_delivery_acceptance_ui_tests`、`aitrain_ocr_segmentation_worker_tests`、`aitrain_platform_tests`。 |
...
进展：只读查询与 Presenter/View 接线已完成。`ProjectQueryServiceV2` 只从
...
写用例由 `ProjectWorkspace`/`ProjectStore` 统一完成，其他页面写入均通过
Worker/Application Service，不再保留第二套 legacy Repository。
...
- fallback translation map 和 `translateWidgetTree`（已删除）。
