...
- Marketplace disable/uninstall is state-safe when Windows keeps an active Qt plugin DLL locked: the GUI releases only recorded marketplace active DLLs, reports `disable-failed` if removal is still blocked, and otherwise updates marketplace state without forcing a hot full-plugin rescan. Use the rescan button to refresh the global plugin matrix count after disable/uninstall.
