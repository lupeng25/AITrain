...
| YOLO Detection | X-AnyLabeling XLABEL | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task yolo2xlabel --mode detect`，需要本机可执行文件。 |
| YOLO Segmentation | COCO JSON | 支持 | 转为 COCO segmentation JSON。 |
| YOLO Segmentation | X-AnyLabeling XLABEL | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task yolo2xlabel --mode segment`。 |
| YOLO OBB | X-AnyLabeling XLABEL | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task yolo2xlabel --mode obb`。 |
| X-AnyLabeling XLABEL | YOLO Detection | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task xlabel2yolo --mode detect`。 |
| X-AnyLabeling XLABEL | YOLO Segmentation | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task xlabel2yolo --mode segment`。 |
| X-AnyLabeling XLABEL | YOLO OBB | 支持，本地 CLI 依赖 | 通过 `xanylabeling convert --task xlabel2yolo --mode obb`。 |
...
- 分类、姿态、异常检测、YOLO-World、YOLOE：不在当前矩阵内。
- X-AnyLabeling Server、远程协作和远程推理 Server：当前不接入，不作为数据集转换能力声明。
...
4. 选择输入路径：
   ...
   - X-AnyLabeling XLABEL：选择 XLABEL 标注目录。
5. 选择输出目录。建议使用新的空目录。
6. 点击“开始转换”。
...
- 可选复制后的图片目录。未复制图片时，输出标签会引用源图片路径或相对路径，迁移包时要特别检查。YOLO -> XLABEL 会把扁平化后的图片持久化到输出目录的 `images` 子目录，并把 XLABEL 的 `imagePath` 改写为相对路径。
- X-AnyLabeling CLI 转换报告会记录 `conversionEngine=xanylabeling_cli`、执行参数、进程输出、本地工具路径、持久化图片目录和 XLABEL 路径改写计数。CLI 转换后的样本/标注计数取决于上游命令返回内容，当前不重新解析 XLABEL 产物做计数承诺。
...
- X-AnyLabeling CLI 是本地外部依赖。缺少可执行文件时转换会失败并写出报告；这表示依赖缺失，不表示 AITrain 原生转换器失败。
