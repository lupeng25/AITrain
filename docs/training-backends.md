...
| `anomalib_efficientad` | Anomaly detection | Anomalib adapter | Secondary anomaly backend. The default preset is `anomalib_efficientad_s` with `modelSize=small`, `lr=0.0001`, `weightDecay=0.00001`, and Anomalib 2.5 training `batchSize=1`. Only `small` or `medium` are accepted; other values are invalid requests. EfficientAD requires `imagenetDir`, `AITRAIN_ANOMALIB_IMAGENET_DIR`, or `.deps/anomalib/imagenette`; missing data is reported as `efficientad_imagenet_dir_missing` / blocked and AITrain does not auto-download external data. |
...
NCNN runtime validation is not a standalone command. Register the converted `.param/.bin` files as a ModelPackage, then run the Runtime Delivery workflow; the workflow records a committed validation report or an explicit unsupported/blocked result.
