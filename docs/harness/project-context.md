...
用户已批准三入口工作台重设计。下文历史段落中的九个并列导航、业务标签和环境页嵌入验收证据已由新方案替代：日常入口为数据集、训练、模型；验收报告由模型工具进入，任务记录为全局工具，环境页保留检查与诊断入口。数据集导入、转换及划分的已提交结果进入项目目录。界面实现规范以 `ui-guidelines.md` 和 `../design/ui-workbench-redesign.md` 为准。


全项目目录搜索、浅/深主题、跨重启训练草稿和英文工作台文案已完成。草稿按 ProjectId 隔离，恢复需重新核对数据身份；搜索条件纳入分页游标。最终验收见 `../design/ui-workbench-closeout.md`。
...
- 当前模块边界：`aitrain_foundation` 已删除，代码收敛到 Product Contract、Storage、Dataset、Model/Runtime、Application、App/Worker 等窄目标。Artifact 的文件提交以同卷 rename 为线性化点，Evidence attach 与 terminalization 状态迁移由 Storage 事务收口；不得重新引入聚合目标或 GUI 反向依赖。
