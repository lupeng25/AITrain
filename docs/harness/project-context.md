...
阶段状态以 `docs/harness/current-status.md` 为准。不要只根据路线图末尾的历史“下一步建议”判断当前阶段。
...
- 插件接口和三个内置插件骨架。
- 数据集校验初版。
- YOLO 分割准入 scaffold：`SegmentationDataset`、polygon-to-mask、overlay 预览、Worker 端 mask 指标和 scaffold checkpoint。
- VSCode 构建、运行、调试配置。
- 第一轮 UI 重构。
...
- 真实 YOLO 分割训练。
- 真实 YOLO 分割 mask head、mask loss 和 segmentation mAP。
- 真实 OCR CTC 训练。
