...
- 下一阶段开发方向已重新确认：补充工业视觉检测中的异常检测/定位、OBB 和专用语义分割能力；其中专用语义分割首版已通过 SMP 路线落地，并已有 RTX 4090D GPU realtest 证据；OBB v1 已接入官方 Ultralytics OBB 和 AITrain ONNX Runtime 旋转框部署路径，并已通过本地 smoke/质量矩阵证据；异常检测/定位仍需实现和验证。
...
- 工业异常检测/定位是下一阶段批准方向但当前仍未实现；OBB v1 已有本地 public DOTA/workflow 质量证据，但客户域工业精度仍需目标域数据验证；专用语义分割已有 SMP 首版闭环和本机 RTX 4090D GPU 实测，但不能把现有 YOLO 检测/实例分割能力描述成语义分割、异常检测或 OBB。
