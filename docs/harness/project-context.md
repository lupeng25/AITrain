...
- `src/core/src/ProductWorkflow.cpp` 已完成第一层 companion 拆分，现在只作为 `ProductWorkflow.h` 的入口锚点；snapshot、quality、evaluation、benchmark、delivery、pipeline 的实现分别位于同目录 `ProductWorkflow*.cpp` 文件，跨文件共享 helper 位于内部 `ProductWorkflowSupport.h/.cpp`。
